...
- A poll for something to appear has no control, so six clean absences meant the walk was broken (an AppleScript `entire contents` walk over a 775-page document's text layer, polled for seventy seconds while the button was on screen throughout --- and it had already begun returning nothing at all, silently; a screenshot of the window is the instrument, and the missing control is that the button's NEIGHBOURS are still found)
- A synthetic click from System Events does not reach the web view, and the pointer was 120 points off (two independent reasons for one symptom, so fixing either alone reads as the fix having been wrong; a screenshot's coordinates are scaled twice over, and `cliclick m:` plus `screencapture -C` draws the cursor into the picture and settles both)
...
- Two mechanisms, one outcome, and deleting either one would have been the defect (the entry above says keep one mechanism; here that would have been the defect, because the two enforce different rules that share one observable -- a listener leak no behaviour can see, closed by counting them, with a second drag and a zero resting count as the two things the count needs)
...
- A widened type enumerates its readers, and is blind to the ones that already had a fallback (twelve compile errors named every site that reads the value and none of the two that coalesce it away; `?? page` and `?? where.page` type-check unchanged because the function always answered `undefined` --- what moved is the *reason*, which no compiler can see, so grep the callers for `??`, `?.`, `unwrap_or` and `else` after widening. Plus: the CLAUSE you add to a conjunction is the hardest thing to test --- two neighbours refused the input first and it took three fixtures before the made page was the only thing that could answer; and a control taken from the untouched document would have agreed with a `select` that could no longer list anything)
- A skip in a reply-driven queue stops the queue, and "nothing was requested" is what a stall looks like (the thumbnail pump is re-entered only by a reply arriving, so a row that needs no request must record itself done *and* pump the next one; TWO tests were wrong --- one asserting `fetchTile` was not called, beside a comment claiming the opposite, and one naming the next row that still passed because the blank page sat at slot 0, where `setActive` pumps twice and the second pump asks for row 1 whatever the first did. Put the skipped item in the MIDDLE, where one pump is all there is)
- A cache keyed by the page of the file, and every caller holding the slot (the doc comment said `viewer.ts` "translates at the four call sites"; there were eighteen calls and three translated, with 1,360 tests and twenty gates green. ONE defect, two opposite symptoms: the drag asked for the right page and read under the wrong key, so no caret could ever be placed; select-all asked for the WRONG page and read under the same wrong key, which is self-consistent, so it selected and copied the deleted page's text on a page spelling something else. A count in a comment is a claim with no check behind it; the remedy is a branded `FilePage` that only `sourceOf` mints, which printed the sixteen call sites the rule never enumerated, and taking the cache away from `selection.ts` so it never meets the distinction)
- A record of what has been asked for, kept where it cannot see the answer being thrown away (`textAsked` had a correct doc comment and no `delete` anywhere; a crop drops the extraction because it is measured in another space, so cropping ANY page of ANY document made it unfetchable and no caret could be placed on it again for the session --- select-all went on working, which reads as flaky rather than broken. The memo belongs beside the thing that FORGETS, and the fact that had to be added is that a failure caches nothing, so "not here" and "asked, and there is nothing" are one observation)
- The status line showed the selection and the copy refused it, over a page with nothing on it (`selectedText` reads `"ab\ncd"` and `copySelection` answers null with *"Some of the selected pages' text could not be read"*, naming two pages that read fine --- a blank page's load and a failed load are the same `null`. Ask `sourceOf` BEFORE the load rather than deriving it from the absence; third time in one increment that one `null` carried two facts, and the sibling path with the identical shape was left alone because no search match can name a blank page)
- One refusal, three callers, one string --- and the string named one of them (`Refusal::MadePage` was raised by a mark, a redaction and a crop, and `describe` turned all three into *"tpdf cannot mark a page it made yet"*; its own doc said "two callers", and the count was the lesser error. One of the three was unreachable behind a frontend guard, one was correct, and the third showed a reader a sentence about an operation they had not performed. For every arm of a refusal-to-string function, COUNT its constructors --- a refusal a reader sees is one fact plus what they were trying to do. Plus the positive half: a `debug_assert` written as a tripwire for "the day this refusal is lifted" is what made lifting it safe)
...
- A mutation harness that dies leaves the mutation in the tree (a `finally` does not survive `pkill`, and on a feature branch the leftover is invisible in `git status`; closed 2026-08-31 by a record written BEFORE the bytes land, so the next run answers by digest --- restore, nothing to do, or refuse because somebody has edited it since)
- A design that wiped the state on every ordinary run was useless in the workflow it was for (the narrow `--only` in the middle of a killed table threw away four hundred proved verdicts, silently; keeping and reusing are separate decisions, and a check that something is PRESERVED needs two subjects, because written with one the run that wiped it recorded it straight back)
- The check aimed at the mutation is the one that raised, and a traceback names no check (a bare `read_text` on the file the mutation stops being written; and the failures printed only at the end went with it, so a run that had caught the defect ten times printed none of them)
- An earlier case emptied the store the later case reads, so neither lookup ever happened (a fingerprint mismatch on purpose two cases earlier; both keys missed and the assertion held for a reason that had nothing to do with keys --- assert the positive direction in the same breath)
...
- Two open questions joined by a word, and the smaller one had no choice in it (the eraser's nib is a hit radius in view pixels and genuinely open; the drawing nib is a width in a file and the format decides it, because a form XObject with no `/Matrix` has the page's space --- two notes weeks apart said "the same open question", which reads as coverage and made an afternoon's work look blocked; ask whether the two share a mechanism or a noun. Plus the coda: at one width a derivation reading the mark and one reading the constant agree on every number, and reading `marks[0]` from two replies compared the narrow rectangle with itself)
