...
The public-fixture edit phase samples replacement, undo and redo pixels without
corrective navigation. A separate `tabs-position` phase checks restoration at nonzero
...
are caught by their named tests. The earlier intermittent offscreen label has not
been independently reproduced; the edit phase now covers that lifecycle directly.
A separate mixed-size rotation regression failed 4 of 8 native checks: refitting
after changing the layout selected the sheet under the stale scroll offset.
