...
`ber::to_definite_length` exists because the old way of finding where a
signature's `/Contents` blob ends --- scan back to the last non-zero byte --- is
wrong whenever the value's own last byte is `0x00`. That is written down under
*A signature blob is trimmed by trailing zero, and BER ends in zeros*, with the
rate: **1 in 256**.

The scan was removed from the code and left standing in the test that checks it:

    let last = raw.iter().rposition(|byte| *byte != 0).expect("not a placeholder");
    assert_eq!(read, raw[..=last], "{name}: DER must come back unchanged");

Its doc comment named the flaw and did not follow it through --- *"the old
trailing-zero scan is the oracle, and it is a fair one for **these fixtures**:
every one ends in a non-zero byte"*. True of the fixtures in front of whoever
wrote it. **The signed fixtures are generated by pyhanko with a fresh key on
every run**, so "these fixtures" is not a fixed set: the last byte of a
signature is effectively random, and `incr-two-signers.pdf` carries two of them.

It cost a release. `v26.8.10-rc1` passed both gate legs; forty minutes later
`v26.8.10` failed `Gates (macos-latest)` on the **same commit**, 788 passed and
1 failed, with `2145` returned against `2144` expected --- one byte, on the
fixture's second signature. Windows passed, because that runner drew different
bytes.

**Two things worth carrying.** A rehearsal tag passing does not predict the real
tag passing when the input is regenerated per run; the two runs are independent
draws, not a repetition. And an oracle written from *the fixtures you can see*
is a claim about a sample, which is only safe when the sample is fixed --- these
are rebuilt on every machine and every run, so a property that held for one
instance is not a property of the fixture at all.

Repaired by taking the expectation from the blob's own outer TLV header: tag,
length, and where that says the value stops. That shares no mechanism with
`to_definite_length`, which walks the whole structure recursively --- reading one
header is five lines --- so it is an independent oracle rather than the writer
agreeing with its own reader. Padding cannot reach it and neither can the
value's last byte.

Proved in both directions rather than by re-running: patching both signatures of
`incr-two-signers.pdf` to end in `0x00` reproduces CI's draw exactly (the two
oracles then differ by one, 2143/2144 against 2144/2145) and the repaired test
passes it; planting a `0xFF` in the padding makes it fail with *"a non-zero byte
