...
uv run --with pypdf scripts/text_continuation_check.py --generate <input.pdf> [--array]
...
def check(before, after):
    ...
    assert ops[0][4:] == [([" SYNTHETIC SECOND"], b"Tj"), ([], b"ET")]
    ...
    assert b"% untouched follower\n( SYNTHETIC SECOND) Tj ET" in pages[1].get_contents().get_data()
...
def main():
    ...
    if args.generate and not args.paths:
        generate(args.generate, args.array)
    elif not args.generate and not args.array and len(args.paths) == 2:
        check(*args.paths)
    else:
