...
`bundleshare` reads every emitted JavaScript chunk after the normal build and
requires zero optional harness implementation. The separate `build:checks`
profile is for native UI probes; it must never be packaged as a normal release.

`fuzz` builds all targets in the independent fuzz package with its locked graph.
This catches stale Plan constructors even when the application compiles cleanly.
...
def gates() -> "list[tuple[str, list[str], str]]":
    ...
    return [
        ...
        (
            "fuzz",
            ["cargo", "build", "--manifest-path", str(ROOT / "src-tauri" / "fuzz" / "Cargo.toml"), "--locked", "--bins"],
            "a fuzz target does not build, or its lockfile is stale",
        ),
        (
            "bins",
            [
        ...
        (
            ...
            "a frontend build profile does not build or loses its checks",
        ),
        (
            "bundleshare",
