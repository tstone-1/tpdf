...
MUTATIONS = [
    ...
    Mutation('refusal kinds: font: misidentify PostScript', 'src/textedit/fonts.rs', 'PostScript Type 1 fonts (FontFile) are not editable yet', 'unsupported embedded CFF font', 'textedit_font_refusals_identify_program_carriers_without_echoing_values'),
    ...
    Mutation('refusal kinds: parent tree: miss object entries', 'src/textedit/tagging.rs', 'crate::encoding::resolve(doc, &pair[1]).as_dict().is_ok()', 'false', 'textedit_parent_tree_refusals_identify_non_page_entries'),
    Mutation('refusal kinds: parent tree: misclassify arrays', 'src/textedit/tagging.rs', 'crate::encoding::resolve(doc, &pair[1]).as_dict().is_ok()', 'crate::encoding::resolve(doc, &pair[1]).as_array().is_ok()', 'textedit_parent_tree_refusals_identify_non_page_entries'),
    ...
    Mutation('tables: allow merged spans', 'src/textedit/tagging/tables.rs', 'integer(span)? != 1', 'false', 'textedit_tables_refuse_spans_metadata_and_attribute_ambiguity'),
    ...
    Mutation('tables: allow orphan rows', 'src/textedit/tagging.rs', 'name(get(node(doc, parent_id)?, b"S")?)? != b"Table"', 'false', 'textedit_tables_require_rows_and_cells_in_their_own_containers'),
    ...
    Mutation('tables: ignore child roles', 'src/textedit/tagging.rs', 'matches!(tag, b"TD" | b"TH")', 'true', 'textedit_tables_require_rows_and_cells_in_their_own_containers'),
    Mutation('tables: allow text in border content', 'src/textedit/tagging.rs', 'self.names[mcid] == b"Table"', 'false', 'textedit_tables_refuse_invalid_hierarchy_and_container_text'),
    ...
    Mutation('tagged first indent: accept document scope', "src/textedit/tagging.rs", 'if name(get(dict, b"S")?)? == b"Document" {', 'if name(get(dict, b"S")?)? == b"Document" && key != b"TextIndent" {', 'textedit_tagged_layout_spacing_rejects_invalid_values_and_document_scope'),
    ...
    Mutation('lists: reject list item blocks', 'src/textedit/tagging.rs', 'if !text_block(role) && !matches!(role, b"LI" | b"TD" | b"TH" | b"Table") {', 'if !text_block(role) && !matches!(role, b"TD" | b"TH" | b"Table") {', 'textedit_lists_preserve_labels_numbering_structure_and_other_items'),
    ...
    Mutation('lists: ignore list child role', 'src/textedit/tagging.rs', 'name(get(node(doc, reference(item)?)?, b"S")?)? != b"LI"', 'false', 'textedit_lists_refuse_wrong_roles_owners_cycles_and_semantic_overrides'),
    ...
    Mutation('lists: allow leaf layout attributes', 'src/textedit/tagging.rs', 'if plain.tag == b"LI" && child.has(b"A") {', 'if false {', 'textedit_lists_refuse_ambiguous_numbering_and_stale_attributes'),
    ...
    Mutation('tag containers: refuse grouping elements', 'src/textedit/tagging.rs', 'if container(role) || matches!(role, b"NonStruct" | b"L" | b"Table" | b"TR") {', 'if false {', 'textedit_containers_and_headings_preserve_roles_order_and_graph'),
    ...
    Mutation('tag containers: permit standard grouping remaps', 'src/textedit/tagging.rs', 'text_block(tag)\n        || container(tag)', 'text_block(tag)\n        || false', 'textedit_containers_refuse_cycles_aliases_attributes_and_wrong_owners_atomically'),
    Mutation('tag containers: permit standard heading remaps', 'src/textedit/tagging.rs', 'text_block(tag)\n        || container(tag)', 'false\n        || container(tag)', 'textedit_containers_refuse_cycles_aliases_attributes_and_wrong_owners_atomically'),
    ...
    Mutation('tag metadata: skip attribute resolution', 'src/textedit/tagging.rs', 'let attributes = crate::encoding::resolve(doc, attributes)\n            .as_dict()', 'let attributes = attributes\n            .as_dict()', 'textedit_tagged_metadata_representations_preserve_the_original_graph'),
    ...
    Mutation('inline tags: route MCIDs through spacer grammar', 'src/textedit.rs', 'inside && !properties.as_dict().is_ok_and(|dict| dict.has(b"MCID"))', 'inside', 'textedit_inline_tags_preserve_continuations_and_every_unedited_operator'),
    ...
    Mutation('role map: omit unsupported figure role', 'src/textedit/tagging.rs', '| b"Figure"', '', 'textedit_role_map_cannot_redefine_any_standard_type_even_when_unused'),
    ...
    Mutation('CFF Unicode: extend unverified CID mappings', 'src/textedit/fonts/mapping.rs', 'target.filter(|_| !matches!(ch, 0x2018 | 0x2019 | 0x2212))', 'target', 'textedit_cff_unicode_mapping_stays_bounded_and_font_specific'),
    ...
    Mutation('span leaves: accept layout attributes', 'src/textedit/tagging.rs', 'matches!(name(get(dict, b"S")?)?, b"NonStruct" | b"Span")', 'name(get(dict, b"S")?)? == b"NonStruct"', 'textedit_spans_refuse_overrides_layout_nesting_and_wrong_owners_atomically'),
    Mutation('nested tags: allow any leaf role', 'src/textedit/tagging.rs', 'if !matches!(tag, b"NonStruct" | b"Span")\n                && !(plain.tag == b"LI" && matches!(tag, b"Lbl" | b"LBody"))\n            {', 'if false {', 'textedit_nested_ownership_cycles_and_extra_levels_are_refused_atomically'),
    ...
    Mutation('nested tags: allow leaf layout attributes', 'src/textedit/tagging.rs', 'if matches!(name(get(dict, b"S")?)?, b"NonStruct" | b"Span") {', 'if false {', 'textedit_nested_metadata_is_bounded_and_never_overrides_replacement_text'),
    ...
    Mutation('browser state: skip stroke colour validation', 'src/textedit.rs', 'colors::values(values, components)?;', 'let _ = components;', 'textedit_stroke_setters_validate_values_without_enabling_stroke_text'),
    ...
    Mutation('reflected: reject intermediate text reflection', 'src/textedit.rs', 'if !orthogonal(matrix) {', 'if !orthogonal(matrix) || matrix[0] < 0. || matrix[3] < 0. {', 'textedit_reflections_cancel_before_hitboxes_and_preserve_other_operators'),
    Mutation('reflected: accept mirrored horizontal text', 'src/textedit.rs', 'if page_matrix[0] * page_matrix[3] - page_matrix[1] * page_matrix[2] <= 0.0 {', 'if page_matrix[0] > 0. && page_matrix[3] < 0. {', 'textedit_reflections_refuse_mirrored_collapsed_skewed_and_unbounded_text_atomically'),
    Mutation('reflected: accept mirrored vertical text', 'src/textedit.rs', 'if page_matrix[0] * page_matrix[3] - page_matrix[1] * page_matrix[2] <= 0.0 {', 'if page_matrix[0] < 0. && page_matrix[3] > 0. {', 'textedit_reflections_refuse_mirrored_collapsed_skewed_and_unbounded_text_atomically'),
    ...
    Mutation('reflected: remove horizontal underflow guard', 'src/textedit.rs', '|| !orthogonal(result)', '|| (!orthogonal(result) && result[0] != 0.)', 'textedit_reflections_refuse_mirrored_collapsed_skewed_and_unbounded_text_atomically'),
    Mutation('reflected: remove vertical underflow guard', 'src/textedit.rs', '|| !orthogonal(result)', '|| (!orthogonal(result) && result[3] != 0.)', 'textedit_reflections_refuse_mirrored_collapsed_skewed_and_unbounded_text_atomically'),
    ...
    Mutation('dash: wrong reverse Unicode', 'src/textedit.rs', "'\\u{2013}'\n    } else {\n        char::from(slot)", 'char::from(slot)\n    } else {\n        char::from(slot)', 'textedit_dash_slots_are_bijective_and_count_characters'),
    ...
    Mutation('mapped: decode bytes without font mapping', 'src/textedit/fonts.rs', 'codes[code as usize].ok_or("text contains an unmapped font code")?', 'code', 'textedit_symbolic_codes_round_trip_without_changing_font_resources'),
    ...
    Mutation('mapped: ignore wrapper operands', 'src/textedit/fonts/mapping.rs', 'actual.operands != expected.operands && !padded_range', 'false', 'textedit_mapping_refuses_ambiguous_partial_and_extended_data'),
