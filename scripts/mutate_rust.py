...
MUTATIONS = [
    ...
    Mutation('room: everything is on the line', 'src/textedit/layout.rs', '        rect[self.cross() + 2].min(high) - rect[self.cross()].max(low) > 0.1', '        true || rect[self.cross() + 2].min(high) - rect[self.cross()].max(low) > 0.1', 'the_room_after_a_run_ignores_what_is_not_on_its_line'),
