...
MUTATIONS = [
    ...
    Mutation('tag containers: discard immediate parent', 'src/textedit/tagging.rs', 'element(doc, child, parent_id, &page_ids)?', 'element(doc, child, root_id, &page_ids)?', 'textedit_containers_keep_interleaved_page_owners_and_nested_leaves'),
    ...
    Mutation('tagged flow: reject preserved empty paragraph', 'src/textedit/tagging.rs', 'if assigned > total {', 'if items.is_empty() || assigned > total {', 'textedit_tagged_flowing_items_refuse_bad_ownership_without_mutation'),
    ...
    Mutation('tagged flow: relax total item limit', 'src/textedit/tagging.rs', 'entries.len() > MAX_CONTENT_ITEMS', 'entries.len() > MAX_CONTENT_ITEMS + 1', 'textedit_tagged_one_paragraph_still_bounds_total_content_items'),
    ...
    Mutation('tagged: offer artifact text for editing', 'src/textedit/tagging.rs', '!self.names.is_empty() && !matches!(self.active, Some(Some(_)))', 'false', 'textedit_tagged_artifact_and_unmarked_text_remain_read_only'),
