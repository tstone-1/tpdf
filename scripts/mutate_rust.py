...
MUTATIONS = [
    ...
    Mutation('wrap room: the line above may not keep its overlap', 'src/textedit/layout.rs', '            allowed[cross] = allowed[cross].max(sliver(other) + 0.001);\n', '', 'text_sliding_along_its_line_may_keep_the_overlap_the_line_above_had'),
    ...
    Mutation('wrap: land on the moved lines', 'src/textedit/layout.rs', '                            shows_of(page, id).iter().any(|show| moving.contains(show)),', '                            false,', 'an_edit_past_the_page_edge_wraps_and_moves_the_rest_of_its_paragraph_down'),
    Mutation('wrap: land on the text that flows', 'src/textedit/layout.rs', '                            shows_of(page, id).iter().any(|show| moving.contains(show)),', '                            shows_of(page, id).iter().any(|show| below.contains(show)),', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    ...
    Mutation('wrap room: land on the next paragraph', 'src/textedit/layout.rs', '            !*moves && wrap::overlaps(new, *other, allowed)\n', '            false && !*moves && wrap::overlaps(new, *other, allowed)\n', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('wrap room: no closer than a tenth of a point', 'src/textedit/layout.rs', '    slack[cross] = (height - pitch).max(0.) + 0.1;', '', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
