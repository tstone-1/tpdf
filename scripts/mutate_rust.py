...
MUTATIONS = [
    ...
    Mutation('inline separators: one space per tab exactly', 'src/textedit/spacers.rs', '!(1..=self.count).contains(&text.len())', 'text.len() != self.count', 'textedit_inline_separators_preserve_bytes_and_positions_without_edit_targets'),
    Mutation('inline separators: any number of spaces', 'src/textedit/spacers.rs', '!(1..=self.count).contains(&text.len())', 'text.is_empty()', 'textedit_inline_separators_refuse_semantics_nesting_and_unbounded_sequences'),
