...
MUTATIONS = [
    ...
    Mutation('read-only fonts: forget the font box', 'src/textedit/fonts.rs', '        vertical_bounds: Some([bottom.min(0.), top.max(0.)]),\n        widths: Box::new([None; 256]),\n        horizontal_overhangs: None,', '        vertical_bounds: None,\n        widths: Box::new([None; 256]),\n        horizontal_overhangs: None,', 'textedit_a_font_that_forbids_editing_keeps_its_text_read_only'),
    Mutation('read-only fonts: an unusable font refuses the page', 'src/textedit.rs', 'read_only_font(doc, resources, name).ok_or(&error)?', 'Option::<fonts::Metrics>::None.ok_or(&error)?', 'textedit_a_font_that_forbids_editing_keeps_its_text_read_only'),
    Mutation('read-only fonts: a page of them gets a generic reason', 'src/textedit.rs', 'unusable_font.get_or_insert(error);', 'let _ = error;', 'textedit_a_font_that_forbids_editing_keeps_its_text_read_only'),
    Mutation('read-only fonts: any box bounds its glyphs', 'src/textedit/fonts.rs', '.any(|v| v.abs() > 4000.) {\n        return None;', '.any(|v| v.abs() > f64::MAX) {\n        return None;', 'textedit_a_font_that_forbids_editing_keeps_its_text_read_only'),
    Mutation('inline separators: an unreadable glyph is no space', 'src/textedit/spacers.rs', "character == ' ' || character == super::fonts::OPAQUE", "character == ' '", 'textedit_a_font_that_forbids_editing_keeps_its_text_read_only'),
