...
MUTATIONS = [
    ...
    Mutation('wrap room: land on the next paragraph', 'src/textedit/layout.rs', '            .any(|(other, moves)| !*moves && lands(*old, new, *other, rule))', '            .any(|(other, moves)| false && !*moves && lands(*old, new, *other, rule))', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    ...
    Mutation('cascade: nothing below moves', 'src/textedit/layout.rs', '            !taken[*index]\n                && rects(shows)', '            !taken[*index]\n                && false\n                && rects(shows)', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation('cascade: every block below moves', 'src/textedit/layout.rs', '            !taken[*index]\n                && rects(shows)', '            !taken[*index]\n                || rects(shows)', 'blocks_below_move_as_far_as_the_first_gap_that_takes_the_added_line'),
    Mutation("cascade: the edit's own lines land on nothing", 'src/textedit/layout.rs', '                        || inks.iter().any(|ink| strikes(*ink, *other, own))', '                        || inks.iter().any(|ink| false && strikes(*ink, *other, own))', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation("cascade: the edit's own box is no exemption", 'src/textedit/layout.rs', '                        || inks.iter().any(|ink| strikes(*ink, *other, own))', '                        || inks.iter().any(|ink| wrap::overlaps(*ink, *other, [0.1, 0.1]) || own[0].is_nan())', 'a_block_the_edited_line_only_grazes_stays'),
    Mutation('cascade: a moved block lands on nothing', 'src/textedit/layout.rs', '        moves.extend(\n            rects(shows)\n                .into_iter()\n                .map(|(show, rect)| (show, rect, by)),\n        );', '        let _ = by;', 'blocks_below_move_as_far_as_the_first_gap_that_takes_the_added_line'),
    Mutation('cascade: a carried block is not written lower', 'src/textedit/layout.rs', '                    for show in &carried {\n', '                    for show in carried.iter().filter(|_| false) {\n', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation('cascade: an edit of a carried block is no conflict', 'src/textedit/layout.rs', '                        .any(|show| carried.contains(&show))', '                        .any(|show| carried.contains(&show) && false)', 'a_wrap_that_moves_the_next_paragraph_refuses_an_edit_of_it'),
    ...
    Mutation('keep breaks: text beside counts', 'src/textedit/layout.rs', '        || new[along + 2].min(other[along + 2]) - new[along].max(other[along]) <= 0.1\n', '', 'a_block_the_edited_line_only_grazes_stays'),
    Mutation('keep breaks: a blank line is a whole pitch', 'src/textedit/layout.rs', '    let blank = pitch - (slack[cross] - 0.1);', '    let blank = pitch;', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('keep breaks: the edited last line is no bottom', 'src/textedit/layout.rs', '(plan.below.is_empty() && drop > 0.).then(', '(false && plan.below.is_empty() && drop > 0.).then(', 'the_edits_new_last_line_keeps_the_break_below_it'),
    ...
    Mutation('keep breaks: the bottom carries nothing', 'src/textedit/layout.rs', '|| edge.is_some_and(|(old, by)| lands(old, shift(old, by), *other, rule))', '|| edge.is_some_and(|_| false)', 'the_edits_new_last_line_keeps_the_break_below_it'),
    ...
    Mutation('keep breaks: the bottom is only as wide as the run', 'src/textedit/layout.rs', '                        old[axis] = old[axis].min(ink[axis]);\n                        old[axis + 2] = old[axis + 2].max(ink[axis + 2]);\n', '                        let _ = ink;\n', 'the_edits_new_last_line_keeps_the_break_below_it'),
