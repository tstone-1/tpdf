...
MUTATIONS = [
    ...
    Mutation('partial editing: omit preserved collision checks', 'src/textedit/layout.rs', 'page.runs.runs.iter().chain(&page.preserved)', 'page.runs.runs.iter()', 'alternate_actual_text_preserves_span_and_refuses_forged_edits_and_overlap'),
    Mutation('partial editing: reject paragraph cells', 'src/textedit/tagging.rs', '&& !(matches!(plain.tag, b"TD" | b"TH") && text_block(tag))', '', 'textedit_tables_preserve_merged_cells_and_paragraph_leaf_ownership'),
    ...
    Mutation('local text preservation: ignore preserved collisions', 'src/textedit/layout.rs', 'page.runs.runs.iter().chain(&page.preserved)', 'page.runs.runs.iter()', 'preserved_skew_text_blocks_layout_collisions'),
    ...
    Mutation('inline tags: route MCIDs through spacer grammar', 'src/textedit.rs', '!properties.as_dict().is_ok_and(|dict| dict.has(b"MCID"))', 'true', 'textedit_inline_tags_preserve_continuations_and_every_unedited_operator'),
