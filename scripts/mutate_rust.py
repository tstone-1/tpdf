...
MUTATIONS = [
    ...
    Mutation('wrap room: refuse a background', 'src/textedit/layout.rs', '            && !swept.iter().all(|rect| wrap::holds(thing, *rect))', '', 'a_drawing_partly_over_the_moved_lines_refuses_and_one_holding_them_does_not'),
    Mutation('wrap room: ignore annotations', 'src/textedit/layout.rs', '    for thing in page.graphics.iter().chain(annotations) {', '    for thing in page.graphics.iter().chain(&annotations[..0]) {', 'an_annotation_over_the_moved_lines_refuses'),
