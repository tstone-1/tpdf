...
MUTATIONS = [
    ...
    Mutation('flow: no gap before a run', 'src/textedit/layout/wrap.rs', '        let mut at = cursor + (unit.start - previous).max(0.);', '        let mut at = cursor;', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    Mutation('flow: keep the gap at the start of a line', 'src/textedit/layout/wrap.rs', '            at = plan.start;\n', '            at = plan.start + (unit.start - previous).max(0.);\n', 'text_after_the_edit_that_does_not_fit_its_last_line_starts_the_next'),
    Mutation('flow: never start a new line', 'src/textedit/layout/wrap.rs', '            line += 1;\n', '', 'text_after_the_edit_that_does_not_fit_its_last_line_starts_the_next'),
    Mutation('flow: a run wider than a line flows anyway', 'src/textedit/layout/wrap.rs', '        if width > plan.rest + 0.000_001 {\n            return None;\n        }\n', '', 'text_after_the_edit_wider_than_a_continuation_line_keeps_the_refusal'),
    ...
    Mutation('wrap: move the lines by the edit\'s own lines only', 'src/textedit/layout.rs', '                    .fold(lines.saturating_sub(1), usize::max);', '                    .fold(lines.saturating_sub(1), |lines, _| lines);', 'text_after_the_edit_that_does_not_fit_its_last_line_starts_the_next'),
    Mutation('wrap: the text that flows stays where it was', 'src/textedit/layout.rs', '                        lowered.push((*show, wrap::lowered(page, *show, along(dx, dy))?));', '                        let _ = along(dx, dy);', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    Mutation('wrap room: the text that flows is not checked or outlined', 'src/textedit/layout.rs', '                        moves.insert(*show, corner(dx, dy));\n', '', 'placements_outline_the_text_that_flowed_after_the_edit'),
    Mutation('wrap: the flowed text lands a line off', 'src/textedit/layout.rs', '                    let (dx, dy) = (at - unit.start, -(*line as f64) * plan.pitch);', '                    let (dx, dy) = (at - unit.start, -(*line as f64 + 1.) * plan.pitch);', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    ...
    Mutation('wrap room: land on the next paragraph', 'src/textedit/layout.rs', '            .any(|(other, moves)| !*moves && wrap::overlaps(new, *other, slack))', '            .any(|(other, moves)| false && !*moves && wrap::overlaps(new, *other, slack))', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
