...
MUTATIONS = [
    ...
    Mutation('source items: never in the layout', 'src/textedit/layout.rs', '    let kept = if fallback.is_none() && !change.replacement.is_empty() {', '    let kept = if false {', 'a_kerned_run_fits_its_own_default_box_unchanged_and_keeps_its_kerns'),
    ...
    Mutation('source items: in a bundled font too', 'src/textedit/layout.rs', '    let kept = if fallback.is_none() && !change.replacement.is_empty() {', '    let kept = if !change.replacement.is_empty() {', 'textedit_a_partly_clipped_run_takes_its_own_text_in_the_default_box'),
    ...
    Mutation('source items: write the rewrite anyway', 'src/textedit/layout.rs', '            Some((items, _, _)) => items.clone(),', '            Some(_) => metrics.items(text, gap)?,', 'a_kerned_run_fits_its_own_default_box_unchanged_and_keeps_its_kerns'),
    Mutation('source items: skip the clip for any kept ink', 'src/textedit/layout.rs', '                    if !kept.is_some_and(|(_, _, inside)| *inside) {', '                    if kept.is_none() {', 'textedit_a_partly_clipped_run_takes_its_own_text_in_the_default_box'),
    Mutation("source items: hold the source's own ink to the clip", 'src/textedit/layout.rs', '                    if !kept.is_some_and(|(_, _, inside)| *inside) {', '                    if true {', 'textedit_a_partly_clipped_run_takes_its_own_text_in_the_default_box'),
    ...
    Mutation('source items: no second try', 'src/textedit/layout.rs', '        Some(Ok(done)) => done,\n        _ => lay_out(None)?,', '        Some(done) => done?,\n        None => lay_out(None)?,', 'textedit_an_edit_the_source_positioning_cannot_place_is_laid_out_afresh'),
    ...
    Mutation('layout: count the space a line broke at as ink', 'src/textedit/layout.rs', "                    let (_, ink) = metrics.gapped_layout(\n                        text.trim_end_matches(' '),", '                    let (_, ink) = metrics.gapped_layout(\n                        text,', 'wrapping_fits_a_line_by_its_words_not_the_space_it_broke_at'),
