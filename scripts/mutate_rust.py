...
MUTATIONS = [
    ...
    Mutation('tag metadata: skip attribute resolution', 'src/textedit/tagging.rs', 'pins |= attributes(doc, tag, crate::encoding::resolve(doc, value))?;', 'pins |= attributes(doc, tag, value)?;', 'textedit_tagged_metadata_representations_preserve_the_original_graph'),
    ...
    Mutation('mapped: write Unicode instead of font codes', 'src/textedit.rs', 'let items = metrics.items(&change.replacement, gap)?;', 'let items = vec![Object::string_literal(encode_text(&change.replacement)?)];', 'textedit_symbolic_codes_round_trip_without_changing_font_resources'),
    ...
    Mutation('mapped: ignore wrapper operands', 'src/textedit/fonts/mapping.rs', 'actual.operands != expected.operands\n                && !padded_range', 'false\n                && !padded_range', 'textedit_mapping_refuses_ambiguous_partial_and_extended_data'),
    ...
    Mutation('alignment: pin a start-aligned block', 'src/textedit/tagging.rs', '            b"Start" => Ok(false),', '            b"Start" => Ok(true),', 'textedit_alignment_pins_a_block_and_line_height_is_kept'),
    ...
    Mutation('placement: require an explicit block placement', 'src/textedit/tagging.rs', '            .get(b"Placement")\n            .is_ok_and(|value| value.as_name().ok() != Some(b"Block"))', '            .get(b"Placement")\n            .map_or(true, |value| value.as_name().ok() != Some(b"Block"))', 'textedit_alignment_pins_a_block_and_line_height_is_kept'),
    Mutation('placement: accept any placement on a block', 'src/textedit/tagging.rs', '            .get(b"Placement")\n            .is_ok_and(|value| value.as_name().ok() != Some(b"Block"))', '            .get(b"Placement")\n            .is_ok_and(|_| false)', 'textedit_alignment_pins_a_block_and_line_height_is_kept'),
    ...
    Mutation('classes: skip class attributes', 'src/textedit/tagging.rs', '                        pins |= attributes(doc, tag, crate::encoding::resolve(doc, object))?;', '                        let _ = object;', 'textedit_classes_apply_the_rules_of_the_elements_own_attributes'),
    ...
    Mutation('type1 maps: accept any label', 'src/textedit/fonts/mapping.rs', '&& !(labelled && label(actual, expected)))', '&& !labelled)', 'textedit_type1_maps_narrow_to_the_repertoire_and_keep_every_code'),
    ...
    Mutation('layout: measure spaces as glyphs', 'src/textedit/layout.rs', ".gapped_layout(text.trim_end_matches(' '), size, spacing, word_spacing, gap)", '.spaced_layout(text, size, spacing, word_spacing)', 'textedit_type1_layout_keeps_the_original_font_and_writes_gaps'),
    ...
    Mutation('kerning: never keep', 'src/textedit.rs', 'let kept = if source.operator == "TJ" && !groups.contains_key(&change.operator) {', 'let kept = if false {', 'textedit_kerning_lets_a_kerned_line_take_an_edit_that_fits_only_with_its_kerns'),
    ...
    Mutation('spacing: ignore spacing when writing', 'src/textedit.rs', '                    run.size,\n                    spacing,\n                    word_spacing,\n                    gap,\n                )?;', '                    run.size,\n                    0.,\n                    word_spacing,\n                    gap,\n                )?;', 'textedit_spacing_replacement_advance_and_ink_are_both_checked_atomically'),
    ...
    Mutation('word spacing: ignore replacement spacing', 'src/textedit.rs', '                    run.size,\n                    spacing,\n                    word_spacing,\n                    gap,\n                )?;', '                    run.size,\n                    spacing,\n                    0.,\n                    gap,\n                )?;', 'textedit_word_spacing_restores_state_and_checks_replacements_atomically'),
