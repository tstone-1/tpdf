...
MUTATIONS = [
    ...
    Mutation('figures: admit editable span children', 'src/textedit/tagging.rs', 'pinned: plain.pinned || pins || in_figure,', 'pinned: plain.pinned || pins,', 'textedit_figure_cannot_expose_text_through_a_span_child'),
    ...
    Mutation('partial editing: omit preserved collision checks', 'src/textedit/layout.rs', '            page.preserved\n                .iter()', '            page.preserved[..0]\n                .iter()', 'alternate_actual_text_preserves_span_and_refuses_forged_edits_and_overlap'),
    ...
    Mutation('local text preservation: ignore preserved collisions', 'src/textedit/layout.rs', '        .chain(\n            page.preserved', '        .chain(\n            page.runs.runs', 'preserved_skew_text_blocks_layout_collisions'),
    ...
    Mutation('tag metadata: ActualText is refused', 'src/textedit/tagging.rs', '    allowed.push(b"ActualText");\n', '', 'textedit_alternate_text_and_titles_pin_the_content_they_describe'),
    Mutation('tag metadata: ActualText pins nothing', 'src/textedit/tagging.rs', '        || dict.has(b"ActualText")\n', '', 'textedit_alternate_text_and_titles_pin_the_content_they_describe'),
    Mutation('tag metadata: ActualText is unbounded', 'src/textedit/tagging.rs', '        (b"ActualText", 65536),', '        (b"ActualText", usize::MAX),', 'textedit_alternate_text_and_titles_pin_the_content_they_describe'),
    Mutation('marked content: a language is unrecognized', 'src/textedit/tagging.rs', 'keys(properties, &[b"MCID", b"Lang"], "marked-content properties")?;', 'keys(properties, &[b"MCID"], "marked-content properties")?;', 'textedit_marked_content_may_name_its_language'),
    Mutation('marked content: any language is a language', 'src/textedit/tagging.rs', '            if let Ok(value) = properties.get(b"Lang") {\n                language(value)?;\n            }', '            if let Ok(value) = properties.get(b"Lang") {\n                let _ = value;\n            }', 'textedit_marked_content_may_name_its_language'),
    ...
    Mutation('inline tags: reset cursor at a tag boundary', 'src/textedit.rs', '                tags.begin(tag, Some(properties))?;\n                if closes(index) {', '                tags.begin(tag, Some(properties))?;\n                cursor = 0.;\n                if closes(index) {', 'textedit_inline_tags_preserve_continuations_and_every_unedited_operator'),
    Mutation('inline tags: accept semantic overrides', 'src/textedit/tagging.rs', 'keys(properties, &[b"MCID", b"Lang"], "marked-content properties")?;', '// unchecked properties', 'textedit_inline_tags_refuse_invalid_semantics_and_balance_atomically'),
    ...
    Mutation('inline separators: one space per tab exactly', 'src/textedit/spacers.rs', '!(1..=self.count).contains(&text.chars().count())', 'text.chars().count() != self.count', 'textedit_inline_separators_preserve_bytes_and_positions_without_edit_targets'),
    Mutation('inline separators: any number of spaces', 'src/textedit/spacers.rs', '!(1..=self.count).contains(&text.chars().count())', 'text.is_empty()', 'textedit_inline_separators_refuse_semantics_nesting_and_unbounded_sequences'),
    Mutation('inline separators: refuse an empty one between text objects', 'src/textedit.rs', 'separator.filter(|_| inside || empty)', 'separator.filter(|_| inside)', 'textedit_an_empty_separator_between_text_objects_is_kept'),
    Mutation('inline separators: an empty one may hold anything', 'src/textedit.rs', 'let empty = !inside && closes(index);', 'let empty = !inside;', 'textedit_an_empty_separator_between_text_objects_is_kept'),
    Mutation('tagged items: an empty item is refused', 'src/textedit.rs', '                if closes(index) {\n                    tags.empty();\n                }', '', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
    Mutation('tagged items: an item holding anything is empty', 'src/textedit.rs', '                if closes(index) {\n                    tags.empty();', '                if true {\n                    tags.empty();', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
    Mutation('symbolic fonts: never read', 'src/textedit.rs', 'fonts::read_only(doc, font).or_else(|| fonts::symbolic(font))', 'fonts::read_only(doc, font)', 'textedit_symbol_and_dingbats_keep_their_text_read_only'),
    Mutation('symbolic fonts: any key may change the codes', 'src/textedit/fonts.rs', '!matches!(key.as_slice(), b"Type" | b"Subtype" | b"BaseFont" | b"Name")', 'false', 'textedit_symbol_and_dingbats_keep_their_text_read_only'),
    Mutation('symbolic fonts: an empty code has a width', 'src/textedit/fonts.rs', '        if *width > 0 {\n            let width = f64::from(*width);', '        if true {\n            let width = f64::from(*width);', 'textedit_symbol_and_dingbats_keep_their_text_read_only'),
    Mutation('read-only fonts: forget the font box', 'src/textedit/fonts.rs', '        vertical_bounds: Some([bottom.min(0.), top.max(0.)]),\n        widths: Box::new([None; 256]),\n        horizontal_overhangs: None,', '        vertical_bounds: None,\n        widths: Box::new([None; 256]),\n        horizontal_overhangs: None,', 'textedit_a_font_tpdf_cannot_read_keeps_its_text_read_only'),
    Mutation('read-only fonts: an unusable font refuses the page', 'src/textedit.rs', 'read_only_font(doc, resources, name).ok_or(&error)?', 'Option::<fonts::Metrics>::None.ok_or(&error)?', 'textedit_a_font_tpdf_cannot_read_keeps_its_text_read_only'),
    Mutation('read-only fonts: a page of them gets a generic reason', 'src/textedit.rs', 'unusable_font.get_or_insert(error);', 'let _ = error;', 'textedit_a_font_tpdf_cannot_read_keeps_its_text_read_only'),
    Mutation('read-only fonts: any box bounds its glyphs', 'src/textedit/fonts.rs', '.any(|v| v.abs() > 4000.) {\n        return None;', '.any(|v| v.abs() > f64::MAX) {\n        return None;', 'textedit_a_font_tpdf_cannot_read_keeps_its_text_read_only'),
    Mutation('inline separators: an unreadable glyph is no space', 'src/textedit/spacers.rs', "character == ' ' || character == super::fonts::OPAQUE", "character == ' '", 'textedit_a_font_tpdf_cannot_read_keeps_its_text_read_only'),
    ...
    Mutation('CFF: omit metadata validation', 'src/textedit/fonts/cff.rs', 'let restricted = profile::validate(&bytes)?.is_some_and(super::restricts);', 'let restricted = false;', 'textedit_cff_refuses_unvalidated_program_semantics_and_permissions'),
    Mutation('CFF: ignore embedding restrictions', 'src/textedit/fonts/cff/profile.rs', 'Ok(i64::from(rights))', 'Ok(0)', 'textedit_cff_metadata_only_accepts_editable_literal_permissions'),
    ...
    Mutation('cid: skip font permissions', 'src/textedit/fonts/composite.rs', 'super::rights_face(&bytes, false)?', '(ttf_parser::Face::parse(&bytes, 0).map_err(|_| INVALID)?, false)', 'textedit_composite_checks_embedding_rights_and_program_format'),
    ...
    Mutation('curves: omit first control point', 'src/textedit/clipping.rs', 'for pair in op.operands.chunks_exact(2) {\n            let x = super::number(&pair[0])?;', 'for pair in op.operands.chunks_exact(2).skip(1) {\n            let x = super::number(&pair[0])?;', 'textedit_curves_refuse_bad_control_points_and_partial_subpaths_atomically'),
    ...
    Mutation('stroked lines: count discard as tagged content', 'src/textedit.rs', 'if content.operations[index + consumed - 1].operator != "n" {', 'if true {', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
    ...
    Mutation('painted rectangles: count discarded paths as tagged content', 'src/textedit.rs', 'if content.operations[index + rectangles_consumed - 1].operator != "n" {', 'if true {', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
    Mutation('painted rectangles: omit tagged paint content', 'src/textedit.rs', 'if content.operations[index + rectangles_consumed - 1].operator != "n" {\n                        tags.paint();\n                    }', '// rectangle paint omitted', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
    ...
    Mutation('tagged pages: select the first page map', 'src/textedit/tagging.rs', '            .remove(&page)\n            .map(|(_, names, blocks)| (names, blocks))', '            .remove(&pages[0])\n            .map(|(_, names, blocks)| (names, blocks))', 'textedit_untagged_page_in_a_tagged_document_edits_as_plain_text'),
    ...
    Mutation('clip: omit rectangle horizontal scale', 'src/textedit/clipping.rs', '(x + width) * ctm[0] + ctm[4]', '(x + width) + ctm[4]', 'textedit_reversed_clip_bounds_normalize_every_corner_after_transform'),
    ...
    Mutation("textedit: ignore present font permissions", "src/textedit/fonts.rs", "restricted = restricts(i64::from(u16::from_be_bytes([rights[0], rights[1]])));", "restricted = restricts(0);", "textedit_macroman_requires_matching_encoding_and_honours_present_rights"),
    ...
    Mutation('pins: forget a leaf pin', 'src/textedit/tagging.rs', 'pinned: plain.pinned || pins || in_figure,', 'pinned: plain.pinned || in_figure,', 'textedit_alternate_text_and_titles_pin_the_content_they_describe'),
    ...
    Mutation('pins: accept an unbounded title', 'src/textedit/tagging.rs', '        (b"T".as_slice(), 4096),\n        (b"Alt", 65536),', '        (b"T".as_slice(), 4097),\n        (b"Alt", 65537),', 'textedit_alternate_text_and_titles_pin_the_content_they_describe'),
    ...
    Mutation('type1: accept restricted rights', 'src/textedit/fonts/type1.rs', 'let restricted = program.rights.is_some_and(super::restricts);', 'let restricted = false;', 'textedit_type1_rights_restrict_editing'),
    ...
    Mutation('room: ignore the clip', 'src/textedit/layout.rs', '            let limit = self.at(self.edge(clip, true));\n            if limit < free {', '            let limit = self.at(self.edge(clip, true));\n            if false {', 'the_room_after_a_run_stops_at_a_clip_and_a_nearer_neighbour_wins'),
    ...
    Mutation("room: a neighbour's far edge faces us", 'src/textedit/layout.rs', '        let near = axis.at(axis.edge(other, false));', '        let near = axis.at(axis.edge(other, true));', 'the_room_after_a_run_reaches_the_page_edge_and_stops_short_of_a_neighbour'),
    Mutation('room: everything is on the line', 'src/textedit/layout.rs', '        shared > 0.1 && shared > shorter / 2.', '        true || shared > 0.1 && shared > shorter / 2.', 'the_room_after_a_run_ignores_what_is_not_on_its_line'),
    Mutation("room: the box's own cross span alone", 'src/textedit/layout.rs', '            boxed[cross].min(own[cross]),\n            boxed[cross + 2].max(own[cross + 2]),', '            boxed[cross],\n            boxed[cross + 2],', 'the_room_after_a_run_counts_a_neighbour_its_own_glyphs_reach'),
    ...
    Mutation('grown box: never grow', 'src/textedit/layout.rs', '        (inherited, settings.grow),', '        (inherited, false),', 'a_box_the_reader_has_not_sized_grows_into_the_free_space_after_the_line'),
    Mutation('grown box: grow a box the reader sized', 'src/textedit/layout.rs', '        (inherited, settings.grow),', '        (inherited, true),', 'a_box_the_reader_has_not_sized_grows_into_the_free_space_after_the_line'),
    Mutation('grown box: say the box is too narrow', 'src/textedit/layout.rs', '    let (too_wide, too_wide_ink) = if settings.grow {', '    let (too_wide, too_wide_ink) = if false {', 'a_grown_box_stops_two_points_short_of_text_it_cannot_move'),
    Mutation('grown box: report the whole room as the box', 'src/textedit/layout.rs', '            inherited + width.max(used).min(ceiling),', '            inherited + ceiling,', 'the_box_reported_back_is_the_size_of_the_text_not_of_the_room_it_had'),
    Mutation('grown box: measure the room, not the text', 'src/textedit/layout.rs', '                used = used.max(advance.max(ink[1]));\n                end = advance.max(ink[1]);', '                used = ceiling;\n                end = advance.max(ink[1]);', 'the_box_reported_back_is_the_size_of_the_text_not_of_the_room_it_had'),
    Mutation('grown box: lay the line out to the box, not the room', 'src/textedit/layout.rs', '            &Shape::boxed(limit, settings.wrap),', '            &Shape::boxed(width, settings.wrap),', 'a_fallback_replacement_grows_into_the_free_space_as_well'),
    ...
    # Pushing the rest of the line along, 26.9.15. Every one of these is a rule
    # the push states in words in `layout.rs` or `docs/TRAPS.md`; a fixture that
    # cannot go red on it is a fixture that agrees with whatever the code does.
    Mutation('push: nothing ever moves', 'src/textedit/layout.rs', '    let moved = if delta > 0. {', '    let moved = if false {', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation('push: move whenever the text is longer', 'src/textedit/layout.rs', '    let delta = (used - free.from).max(0.);', '    let delta = (used - run.advance).max(0.);', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation('push: measure the move from the origin', 'src/textedit/layout.rs', '    let delta = (used - free.from).max(0.);', '    let delta = used.max(0.);', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation('push: measure the move from the room instead', 'src/textedit/layout.rs', '    let from = (near - (near - own_far).clamp(0., space)).max(width);', '    let from = free;', 'the_push_is_measured_from_the_run_it_moves_not_from_the_room'),
    Mutation('push: the wrong way along the line', 'src/textedit/layout.rs', '    let written = (existing - shift / per_unit) as f32;', '    let written = (existing + shift / per_unit) as f32;', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation("push: forget the source's own displacement", 'src/textedit/layout.rs', '    let existing = if leading { number(&items[0])? } else { 0. };', '    let existing = 0.;', 'a_neighbour_that_already_opens_with_a_displacement_keeps_it'),
    Mutation('push: add a second leading number', 'src/textedit/layout.rs', '    if leading {\n        items[0] = Object::Real(written);\n    } else {', '    if false {\n        items[0] = Object::Real(written);\n    } else {', 'a_neighbour_that_already_opens_with_a_displacement_keeps_it'),
    Mutation('push: any displacement a float can hold', 'src/textedit/layout.rs', '        || f64::from(written).abs() > 1_000_000.', '        || false', 'a_displacement_beyond_a_pdf_number_refuses_the_push'),
    Mutation('push: no reach beyond the run that moves', 'src/textedit/layout.rs', '        if best.0 < shift {\n            (shift, stop) = best;', '        if false {\n            (shift, stop) = best;', 'the_page_edge_stops_the_push_beyond_the_run_that_moves'),
    Mutation('push: the page does not bound the run that moves', 'src/textedit/layout.rs', '        let (limit, kind) = axis.ahead(sheet, *clip);', '        let (limit, kind) = axis.ahead([-1e6, -1e6, 1e6, 1e6], *clip);', 'the_page_edge_stops_the_push_beyond_the_run_that_moves'),
    Mutation('push: ignore the clip over the run that moves', 'src/textedit/layout.rs', '            page.contexts[&other.operator]\n                .clip\n                .map(|clip| display(clip).map(f64::from)),', '            None,', 'a_clip_over_the_run_that_moves_stops_the_push_there'),
    Mutation('push: drawings are not in the way', 'src/textedit/layout.rs', '    let mut fixed: Vec<([f64; 4], Room)> = page\n        .graphics\n        .iter()', '    let mut fixed: Vec<([f64; 4], Room)> = page\n        .graphics[..0]\n        .iter()', 'a_drawing_after_the_run_that_moves_stops_the_push_but_not_the_box'),
    Mutation('push: a path that paints nothing is a drawing', 'src/textedit/clipping.rs', '    if end.operator == "n" {\n        return Ok(None);', '    if false {\n        return Ok(None);', 'a_path_that_paints_nothing_is_not_a_drawing'),
    Mutation('push: nothing separates two runs', 'src/textedit/layout.rs', '        if !axis.beside(band, rect) || near >= hard || near + 0.1 < own_far {', '        if !axis.beside(band, rect) || near + 0.1 < own_far {', 'a_run_beyond_text_that_cannot_move_is_left_alone'),
    Mutation('push: stream order says nothing about sides', 'src/textedit/layout.rs', '        if !axis.beside(band, rect) || near >= hard || near + 0.1 < own_far {', '        if !axis.beside(band, rect) || near >= hard {', 'a_run_behind_the_edit_on_its_own_line_is_not_pushed'),
    Mutation('push: a run drawn earlier may be moved', 'src/textedit/layout.rs', '    other > run.operator\n        && shows', '    true\n        && shows', 'a_run_drawn_before_the_edit_is_not_pushed'),
    Mutation('push: displace every show, carried or not', 'src/textedit/layout.rs', '                    if !displaced {\n                        moved.push((show, shift));\n                    }', '                    moved.push((show, shift));', 'a_continued_show_is_carried_by_the_cursor_and_not_pushed_twice'),
    Mutation('push: a position does not reset the cursor', 'src/textedit/layout.rs', '            "Td" | "TD" | "T*" | "Tm" | "BT" => displaced = false,', '            "Td" | "TD" | "T*" | "Tm" | "BT" => {}', 'a_grouped_run_moves_with_every_show_it_was_built_from'),
    Mutation('push: a grouped member stays behind', 'src/textedit/layout.rs', '            page.groups\n                .get(&operator)\n                .map_or(&[][..], Vec::as_slice)', '            page.groups\n                .get(&operator)\n                .map_or(&[][..], |_| &[][..])', 'a_grouped_run_moves_with_every_show_it_was_built_from'),
    Mutation('push: drag anything the cursor reaches', 'src/textedit/layout.rs', '                } else if displaced && !free.carry.contains(&show) {', '                } else if false {', 'an_invisible_read_only_show_on_the_cursor_refuses_the_push'),
    Mutation('push: a replaced show carries the push on', 'src/textedit/layout.rs', '                if placement.edited.contains(&show) {\n                    displaced = false;', '                if false {\n                    displaced = false;', 'an_edit_in_the_middle_of_a_pushed_line_does_not_carry_the_push'),
    Mutation('push: an edit does not start where the last one left it', 'src/textedit.rs', '                inherited: shifts\n                    .get(&(change.page, change.operator))\n                    .copied()\n                    .unwrap_or_default(),', '                inherited: 0.,', 'two_edits_on_one_line_compose'),
    Mutation('push: write an operand for every show on the line', 'src/textedit.rs', '    for ((page, operator), shift) in &pushes {', '    for ((page, operator), shift) in &shifts {', 'a_continued_show_is_carried_by_the_cursor_and_not_pushed_twice'),
    Mutation('push: equal limits mean nothing movable is in the way', 'src/textedit/layout.rs', '    let band = axis.band(edges.0, own);\n    // Stream order alone', '    if free >= hard {\n        return nothing(stop);\n    }\n    let band = axis.band(edges.0, own);\n    // Stream order alone', 'a_flush_neighbour_with_nothing_after_it_is_still_pushed'),
    Mutation('push: the box keeps the room past a skipped run', 'src/textedit/layout.rs', '        (from + shift).max(free.min(from)),', '        (from + shift).max(free),', 'a_flush_neighbour_with_nothing_after_it_is_still_pushed'),
    Mutation('push: a frame around the run stops it', 'src/textedit/layout.rs', '            let room = if *kind == Room::Drawn && near <= axis.at(axis.edge(*rect, false)) {', '            let room = if false {', 'a_drawing_holding_the_run_that_moves_allows_it_to_its_far_edge'),
    Mutation('push: any drawing over the run holds it', 'src/textedit/layout.rs', '            let room = if *kind == Room::Drawn && near <= axis.at(axis.edge(*rect, false)) {', '            let room = if *kind == Room::Drawn {', 'a_drawing_holding_the_run_that_moves_allows_it_to_its_far_edge'),
    Mutation('push: the preview crop stops at the box', 'src/textedit/layout.rs', '    for other in placed.iter().map(|(_, other)| other).chain(', '    for other in placed[..0].iter().map(|(_, other)| other).chain(', 'the_preview_extent_covers_the_run_the_draft_pushes'),
    Mutation('wrap: the preview crop leaves out where the flowed text was', 'src/textedit/layout.rs', '.chain(&vacated)', '.chain(&vacated[..0])', 'the_preview_covers_where_the_text_that_flowed_was'),
    Mutation("wrap: the preview crop leaves out the lines it moved", 'src/textedit/layout.rs', '    placed.extend(down);\n', '', 'the_preview_covers_where_the_moved_lines_were_as_well_as_where_they_went'),
    # Which runs share a line (`Axis::beside`): the box, the push and what stops it.
    Mutation('same line: any overlap is the line', 'src/textedit/layout.rs', '        shared > 0.1 && shared > shorter / 2.', '        shared > 0.1', 'the_next_line_is_not_pushed_along_with_this_one'),
    Mutation('same line: all of it or nothing', 'src/textedit/layout.rs', '        shared > 0.1 && shared > shorter / 2.', '        shared > 0.1 && shared >= shorter', 'the_room_after_a_run_ignores_what_is_not_on_its_line'),
    Mutation('same line: measured against the box alone', 'src/textedit/layout.rs', '        let shorter = (rect[cross + 2] - rect[cross]).min(high - low);', '        let shorter = high - low;', 'the_room_after_a_run_ignores_what_is_not_on_its_line'),
    Mutation('same line: the box stops at the next line', 'src/textedit/layout.rs', '        if !axis.beside(band, other) {', '        if !(other[axis.cross() + 2].min(band.1) - other[axis.cross()].max(band.0) > 0.1) {', 'the_room_after_a_run_ignores_what_is_not_on_its_line'),
    Mutation('same line: a push stopped by the next line', 'src/textedit/layout.rs', '            if !axis.beside(span, *other) ||', '            if !(other[axis.cross() + 2].min(span.1) - other[axis.cross()].max(span.0) > 0.1) ||', 'text_on_the_next_line_does_not_stop_a_push'),
    Mutation('same line: the next line pushed along', 'src/textedit/layout.rs', '        if !axis.beside(band, rect) || near >= hard', '        if !(rect[axis.cross() + 2].min(band.1) - rect[axis.cross()].max(band.0) > 0.1) || near >= hard', 'the_next_line_is_not_pushed_along_with_this_one'),
    # Wrapping onto a new line of the paragraph, on a tagged page (`layout::wrap`).
    Mutation('wrap tags: no block owns the content', 'src/textedit/tagging.rs', '                    blocks[mcid] = Some(block);\n', '', 'geometry_answers_only_where_the_tags_do_not'),
    Mutation('wrap tags: the scan records no block', 'src/textedit.rs', '                if let Some(block) = tags.block() {\n                    blocks.insert(index as u32, block);\n                }', '', 'geometry_answers_only_where_the_tags_do_not'),
    Mutation('wrap: text after the run stays where it is', 'src/textedit/layout/wrap.rs', '            if x > 0. && !other.text.trim().is_empty() {', '            if false {', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    Mutation('wrap: flow text that cannot be moved', 'src/textedit/layout/wrap.rs', '                if !page.contexts.contains_key(&show)\n                    || page.actual_text.contains_key(&show)\n                    || page.compound_run_clips.contains_key(&show)\n                {\n                    return Err(Refused::NotApplicable);', '                if false {\n                    return Err(Refused::NotApplicable);', 'text_after_the_edit_that_cannot_be_moved_keeps_the_refusal'),
    Mutation('wrap: flow text only the push saw', 'src/textedit/layout/wrap.rs', '                if !page.contexts.contains_key(&show)\n                    || page.actual_text', '                if !line.contains(&show)\n                    || page.actual_text', 'text_kerned_into_the_edit_flows_although_the_push_leaves_it'),
    Mutation("wrap: flow another block's text", 'src/textedit/layout/wrap.rs', '    if !after.is_empty()\n        && !line', '    if false\n        && !line', 'another_blocks_text_after_the_edit_on_its_line_keeps_the_page_edge_refusal'),
    Mutation('wrap: the first line stops where the text after it was', 'src/textedit/layout/wrap.rs', '    let first = if after.is_empty() { room } else { whole };', '    let first = room;', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    Mutation('flow: follow the advance, not an overhanging ink', 'src/textedit/layout.rs', '                end = start + (inset + advance).max(ink[1] + inset);', '                end = start + inset + advance;', 'text_after_the_edit_keeps_its_gap_from_an_overhanging_last_glyph'),
    Mutation('flow: no gap before a run', 'src/textedit/layout/wrap.rs', '        let at = cursor + (unit.start - previous).max(0.);', '        let at = cursor;', 'the_text_after_the_edit_on_its_line_flows_after_it'),
    Mutation('flow: keep the gap at the start of a line', 'src/textedit/layout/wrap.rs', '                line += 1;\n                plan.start\n', '                line += 1;\n                plan.start + (unit.start - previous).max(0.)\n', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('flow: never start a new line', 'src/textedit/layout/wrap.rs', '                        (line, at, fresh) = (line + 1, plan.start, true);', '                        (at, fresh) = (plan.start, true);', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('flow: a run wider than a line flows anyway', 'src/textedit/layout/wrap.rs', '                None if fresh => return None,', '                None if fresh => break,', 'text_after_the_edit_wider_than_a_continuation_line_is_cut_or_refused'),
    Mutation('flow: never cut a run', 'src/textedit/layout/wrap.rs', '        let spans = words.get(unit_index).map_or(&[][..], Vec::as_slice);', '        let spans: &[(f64, f64)] = &[];', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('flow: a cut run keeps its leading space as a gap', 'src/textedit/layout/wrap.rs', '        let mut at = cursor + (spans[0].0 - previous).max(0.);', '        let mut at = cursor + (unit.start - previous).max(0.);', 'text_kerned_into_the_edit_flows_although_the_push_leaves_it'),
    Mutation('flow: measure the next run from the cut run\'s advance', 'src/textedit/layout/wrap.rs', '        previous = spans[spans.len() - 1].1;', '        previous = unit.end;', 'the_run_after_a_cut_one_keeps_the_space_it_ended_with'),
    Mutation('flow: a word that does not fit is placed anyway', 'src/textedit/layout/wrap.rs', '                .take_while(|&last| fits(at, spans[last].1 - spans[next].0))', '                .take_while(|_| true)', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('cut: the words of a piece run together', 'src/textedit/kerning.rs', '        let before = if position > 0 {\n            &word.before[..]', '        let before = if false {\n            &word.before[..]', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('cut: two spaces in a row are one', 'src/textedit/kerning.rs', '        if index < atoms.len() && index + 1 < atoms.len() && space(&atoms[index + 1]) {\n            return None;\n        }\n', '', 'textedit_type1_words_cut_at_word_gaps_and_keep_their_kerns'),
    Mutation('cut: a word keeps the kerns beside it', 'src/textedit/kerning.rs', '        emit(&atoms[first..=last], &mut items);\n        let text', '        emit(&atoms[start..index], &mut items);\n        let text', 'textedit_type1_words_cut_at_word_gaps_and_keep_their_kerns'),
    Mutation('cut: a piece is drawn as the whole run', 'src/textedit/layout/wrap.rs', '            Some(items) => Operation::new("TJ", vec![Object::Array(items.to_vec())]),', '            Some(_) => operation.clone(),', 'text_after_the_edit_that_does_not_fit_its_last_line_is_cut_at_a_space'),
    Mutation('cut: a piece is checked as the whole run', 'src/textedit/layout.rs', '                        moving.push((show, rect, corner(piece.at - from, dy(piece))));', '                        moving.push((show, source, corner(piece.at - from, dy(piece))));', 'placements_outline_both_pieces_of_a_cut_run'),
    ...
    Mutation('wrap: continue at the run', 'src/textedit/layout/wrap.rs', '        .fold(f64::INFINITY, f64::min);\n    let rest = far - start;', '        .fold(f64::INFINITY, f64::min)\n        .max(0.);\n    let rest = far - start;', 'a_continuation_starts_at_the_paragraphs_left_edge_under_a_first_line_indent'),
    ...
    Mutation('wrap: at another size', 'src/textedit/layout.rs', '    if full && placement.inherited == 0. && size == run.size {', '    if full && placement.inherited == 0. {', 'a_different_size_or_a_run_already_pushed_keeps_the_page_edge_refusal'),
    Mutation('wrap: after a push', 'src/textedit/layout.rs', '    if full && placement.inherited == 0. && size == run.size {', '    if full && size == run.size {', 'a_different_size_or_a_run_already_pushed_keeps_the_page_edge_refusal'),
    ...
    Mutation('wrap: new lines past the foot of the page', 'src/textedit/layout.rs', '                if shape.pitch.is_some()\n                    && (shown[0] < -0.001', '                if false\n                    && (shown[0] < -0.001', 'at_the_foot_of_the_page_there_is_no_room_below'),
    Mutation('wrap graze: every overlap strikes', 'src/textedit/layout.rs', '        && !graze\n', '\n', 'a_wrap_on_lines_closer_than_their_boxes_are_tall_wraps'),
    Mutation('wrap graze: any overlap grazes', 'src/textedit/layout.rs', 'const GRAZE: f64 = 0.125;', 'const GRAZE: f64 = 0.5;', 'a_wrap_on_lines_closer_than_their_boxes_are_tall_wraps'),
    Mutation('wrap graze: no overlap grazes', 'src/textedit/layout.rs', 'const GRAZE: f64 = 0.125;', 'const GRAZE: f64 = 0.;', 'a_wrap_on_lines_closer_than_their_boxes_are_tall_wraps'),
    Mutation('wrap links: any annotation is a link', 'src/textedit.rs', '.is_ok_and(|kind| kind == b"Link")', '.is_ok_and(|_| true)', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: one with an appearance moves', 'src/textedit.rs', '                    && !annotation.has(b"AP")\n', '\n', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: one with quadrilaterals moves', 'src/textedit.rs', '                    && !annotation.has(b"QuadPoints") =>', '                    =>', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: the new rectangle is not written', 'src/textedit.rs', '        for (link, rect) in links {', '        for (link, rect) in links.into_iter().take(0) {', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: the rectangle keeps its source numbers', 'src/textedit.rs', '.map(|i| source[i] + now[i] - was[i]);', '.map(|i| source[i]);', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: the move is not turned with the page', 'src/textedit.rs', '.map(|i| source[i] + now[i] - was[i]);', '.map(|i| source[i] + [by[0], -by[1], by[0], -by[1]][i] + 0. * (now[i] - was[i]));', 'a_link_moves_with_its_line_on_a_turned_page'),
    Mutation('wrap links: a held link is left behind', 'src/textedit/layout.rs', '            Some(moved) => links.push(moved),', '            Some(_) => {}', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: a link must lie within the run', 'src/textedit/layout.rs', 'const LINK_SLACK: f64 = 2.0;', 'const LINK_SLACK: f64 = 1.0;', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('wrap links: a link may reach well past the run', 'src/textedit/layout.rs', 'const LINK_SLACK: f64 = 2.0;', 'const LINK_SLACK: f64 = 3.0;', 'a_link_over_a_line_the_wrap_moves_moves_with_it'),
    Mutation('columns: no column is exempt from the level check', 'src/textedit/layout.rs', '!*moves && !columns.contains(other) && level(', '!*moves && level(', 'a_column_beside_the_paragraph_does_not_refuse_the_wrap_on_an_untagged_page'),
    Mutation('columns: only on pages without tags', 'src/textedit/layout.rs', '            .unwrap_or_default(),\n        None => BTreeSet::new(),\n    };', '            .filter(|_| blocks::geometric_page(page))\n            .unwrap_or_default(),\n        None => BTreeSet::new(),\n    };', 'a_tagged_column_is_not_pushed_along_the_line_beside_it'),
    Mutation('gutter: a line may fill it', 'src/textedit/layout.rs', '    if side >= near {', '    if true {', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('gutter: the run itself is the measure', 'src/textedit/layout.rs', '.fold(own_far, f64::max);', '.fold(own_far, |measure, _| measure);', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('gutter: any line on the page is the measure', 'src/textedit/layout.rs', '                && rect[cross + 2] > top\n', '                && rect[cross + 2] > f64::NEG_INFINITY\n', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('gutter: a line may take all of it', 'src/textedit/layout.rs', 'const GUTTER_SHARE: f64 = 0.5;', 'const GUTTER_SHARE: f64 = 0.9;', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('gutter: a line may take none of it', 'src/textedit/layout.rs', 'const GUTTER_SHARE: f64 = 0.5;', 'const GUTTER_SHARE: f64 = 0.;', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('gutter: the push may enter it', 'src/textedit/layout.rs', '    fixed.extend(gutter.map(|rect| (rect, Room::Column)));', '', 'a_line_stops_before_the_gutter_and_wraps_at_its_columns_measure'),
    Mutation('columns: a full line leaves the wrap the room before the next run', 'src/textedit/layout.rs', 'if shift <= 0. && pushed_stop != Room::Column {', 'if shift <= 0. {', 'a_tagged_column_is_not_pushed_along_the_line_beside_it'),
    Mutation('push: the gap is spent before anything moves', 'src/textedit/layout.rs', '.clamp(0., space)', '.clamp(0., 0.)', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation('push: a gap is kept whatever its width', 'src/textedit/layout.rs', '.clamp(0., space)', '.max(0.)', 'the_next_run_on_the_line_moves_by_what_the_text_ran_past_the_room'),
    Mutation('columns: two lines make a column', 'src/textedit/layout.rs', 'const COLUMN_LINES: usize = 3;', 'const COLUMN_LINES: usize = 2;', 'a_column_beside_the_paragraph_does_not_refuse_the_wrap_on_an_untagged_page'),
    Mutation('columns: a narrow block is a column', 'src/textedit/layout.rs', '            lines.len() >= COLUMN_LINES && high - low >= width / 2.', '            lines.len() >= COLUMN_LINES', 'a_column_beside_the_paragraph_does_not_refuse_the_wrap_on_an_untagged_page'),
    Mutation('columns: a column owns nothing set within it', 'src/textedit/layout.rs', '        .chain(owned)\n', '\n', 'a_short_line_within_a_column_beside_the_paragraph_does_not_refuse_the_wrap'),
    Mutation('columns: a block across the paragraph owns what is within it', 'src/textedit/layout.rs', '        .filter(|[low, high]| *high <= paragraph[0] || *low >= paragraph[1])\n', '\n', 'a_short_line_within_a_column_beside_the_paragraph_does_not_refuse_the_wrap'),
    Mutation('columns: all short text is within a column', 'src/textedit/layout.rs', '.any(|side| low >= side[0] - COLUMN_SLACK && high <= side[1] + COLUMN_SLACK)', '.any(|_| true)', 'a_short_line_within_a_column_beside_the_paragraph_does_not_refuse_the_wrap'),
    Mutation("columns: a heading must start at the column's edge", 'src/textedit/layout.rs', 'const COLUMN_SLACK: f64 = 1.0;', 'const COLUMN_SLACK: f64 = 0.;', 'a_short_line_within_a_column_beside_the_paragraph_does_not_refuse_the_wrap'),
    Mutation('columns: a column reaches into its gutter', 'src/textedit/layout.rs', 'const COLUMN_SLACK: f64 = 1.0;', 'const COLUMN_SLACK: f64 = 10.;', 'a_short_line_within_a_column_beside_the_paragraph_does_not_refuse_the_wrap'),
    Mutation('columns: a column is pushed along', 'src/textedit/layout.rs', 'if grow && pushable(page, run, id) && !columns.contains(&id) =>', 'if grow && pushable(page, run, id) =>', 'a_tagged_column_is_not_pushed_along_the_line_beside_it'),
    Mutation('columns: a line stops at a column as at other text', 'src/textedit/layout.rs', '        if beyond > free + 1e-9 {\n            Room::Column\n', '        if beyond > free + 1e-9 {\n            stop\n', 'a_line_that_reaches_the_next_column_wraps_on_an_untagged_page'),
    Mutation('columns: a push stops at a column as at other text', 'src/textedit/layout.rs', '                        Around::Run(id, _) if columns.contains(&id) => Room::Column,\n', '', 'a_tagged_column_is_not_pushed_along_the_line_beside_it'),
    Mutation('columns: only the page edge wraps', 'src/textedit/layout.rs', '        Err(error) if error == Room::Column.refusal() => Some(Room::Column),\n', '', 'a_line_that_reaches_the_next_column_wraps_on_an_untagged_page'),
    Mutation('wrap room: moved lines past the foot of the page', 'src/textedit/layout.rs', '        if new[0] < -0.001 || new[1] < -0.001 || new[2] > width + 0.001 || new[3] > depth + 0.001', '        if false', 'at_the_foot_of_the_page_there_is_no_room_below'),
    Mutation('wrap room: land on the next paragraph', 'src/textedit/layout.rs', '            .any(|(other, moves)| !*moves && lands((*old, new, *reach), *other, rule))', '            .any(|(other, moves)| false && !*moves && lands((*old, new, *reach), *other, rule))', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('wrap room: no closer than a tenth of a point', 'src/textedit/layout.rs', '    slack[cross] = (height - down[cross].abs()).max(0.) + 0.1;', '', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('wrap room: ignore drawings', 'src/textedit/layout.rs', '            .any(|rect| wrap::overlaps(*rect, thing, [0.1, 0.1]))', '            .any(|rect| false && wrap::overlaps(*rect, thing, [0.1, 0.1]))', 'a_drawing_partly_over_the_moved_lines_refuses_and_one_holding_them_does_not'),
    Mutation('wrap room: refuse a background', 'src/textedit/layout.rs', '            || swept.iter().all(|rect| wrap::holds(thing, *rect))\n', '\n', 'a_drawing_partly_over_the_moved_lines_refuses_and_one_holding_them_does_not'),
    Mutation('wrap room: ignore annotations', 'src/textedit/layout.rs', '        annotations\n            .iter()\n            .map(|a| (a.rect, a.link.map(|(id, _)| id))),', '        annotations[..0]\n            .iter()\n            .map(|a| (a.rect, a.link.map(|(id, _)| id))),', 'an_annotation_over_the_moved_lines_refuses'),
    Mutation('wrap room: trust an unreadable annotation list', 'src/textedit/layout.rs', '    let annotations = page.annotations.as_deref().ok_or(wrap::DRAWN)?;', '    let annotations = page.annotations.as_deref().unwrap_or_default();', 'an_unreadable_annotation_list_refuses_a_wrap_and_nothing_else'),
    Mutation('wrap room: a clip does not hold', 'src/textedit/layout.rs', '        if clip.is_some_and(|clip| !wrap::holds(clip, new)) {', '        if false {', 'a_clip_over_the_paragraph_stops_the_lines_it_would_move'),
    Mutation('cascade: nothing below moves', 'src/textedit/layout.rs', '            if far > gone.get(&index).copied().unwrap_or(0.) + NO_MOVE {', '            if false && far > gone.get(&index).copied().unwrap_or(0.) + NO_MOVE {', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation('cascade: every block below moves', 'src/textedit/layout.rs', '            let mut far = 0_f64;', '            let mut far = full;', 'blocks_below_move_as_far_as_the_first_gap_that_takes_the_added_line'),
    Mutation("cascade: the edit's own lines land on nothing", 'src/textedit/layout.rs', '                if inks.iter().any(|ink| strikes(*ink, *other, own)) {', '                if inks.iter().any(|ink| false && strikes(*ink, *other, own)) {', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation("cascade: the edit's own box is no exemption", 'src/textedit/layout.rs', '                if inks.iter().any(|ink| strikes(*ink, *other, own)) {', '                if inks.iter().any(|ink| strikes(*ink, *other, [0.; 4])) {', 'a_block_the_edited_line_only_grazes_stays'),
    ...
    Mutation('keep breaks: text beside counts', 'src/textedit/layout.rs', '        || reach[1].min(other[along + 2]) - reach[0].max(other[along]) <= 0.1\n', '', 'a_block_the_edited_line_only_grazes_stays'),
    Mutation('keep breaks: a blank line is a whole pitch', 'src/textedit/layout.rs', '    let blank = pitch - (slack[cross] - 0.1);\n    ahead && !level && after < blank - 0.1', '    let blank = pitch;\n    ahead && !level && after < blank - 0.1', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('keep breaks: a run set off its own line is ahead of it', 'src/textedit/layout.rs', '    ahead && !level && after < blank - 0.1', '    ahead && after < blank - 0.1', 'a_lowered_run_before_the_edit_is_not_below_the_text_that_flows'),
    Mutation('keep breaks: everything is level', 'src/textedit/layout.rs', '    let level = shared >= shorter / 2.;', '    let level = true;', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    Mutation('keep breaks: the edited last line is no bottom', 'src/textedit/layout.rs', '(plan.below.is_empty() && drop > 0.)\n', '(false && plan.below.is_empty() && drop > 0.)\n', 'the_edits_new_last_line_keeps_the_break_below_it'),
    Mutation('keep breaks: the room check ignores the bottom', 'src/textedit/layout.rs', '    if let Some((old, by)) = edge {', '    if let Some((old, by)) = edge.filter(|_| false) {', 'the_edits_new_last_line_keeps_the_break_below_it'),
    Mutation('keep breaks: the bottom carries nothing', 'src/textedit/layout.rs', '        .chain(edge.map(|(old, by)| (old, by[cross] * sign)))', '        .chain(edge.filter(|_| false).map(|(old, by)| (old, by[cross] * sign)))', 'the_edits_new_last_line_keeps_the_break_below_it'),
    Mutation('keep breaks: the bottom has no height', 'src/textedit/layout.rs', '        .chain(edge.map(|(rect, _)| rect))', '        .chain(edge.map(|(rect, _)| rect).filter(|_| false))', 'the_edits_new_last_line_keeps_the_break_below_it'),
    Mutation("spread: a break's spare is measured to a full pitch", 'src/textedit/layout.rs', '    let blank = pitch - (slack[cross] - 0.1);\n    // Positions along', '    let blank = pitch;\n    // Positions along', 'the_added_lines_are_spread_over_the_breaks_below'),
    ...
    Mutation('spread: a moved block moves nothing after it', 'src/textedit/layout.rs', '                    if *moved != index {', '                    if *moved != index && false {', 'blocks_below_move_as_far_as_the_first_gap_that_takes_the_added_line'),
    Mutation('spread: it never settles', 'src/textedit/layout.rs', '        if !changed {\n            settled = true;', '        if !changed {\n            settled = false;', 'the_next_paragraph_moves_down_when_the_lines_would_land_on_it'),
    Mutation('spread: text beside is ahead too', 'src/textedit/layout.rs', '        if ahead && shared > 0.1 && other[cross + 2] - other[cross] > 0.1 {', '        if ahead && shared > -f64::MAX && other[cross + 2] - other[cross] > 0.1 {', 'a_block_the_edited_line_only_grazes_stays'),
    Mutation('spread: text clipped away keeps a break', 'src/textedit/layout.rs', '        if ahead && shared > 0.1 && other[cross + 2] - other[cross] > 0.1 {', '        if ahead && shared > 0.1 {', 'a_clip_over_the_paragraph_stops_the_lines_it_would_move'),
    Mutation('spread: text behind is ahead too', 'src/textedit/layout.rs', '        let ahead = start(other) + end(other) > start(old) + end(old);', '        let ahead = true;', 'a_block_the_edited_line_only_grazes_stays'),
    Mutation("spread: a line's reach is its own", 'src/textedit/layout.rs', '                    far = far.max(need(*old, *went, paragraph, *other, reaches[index]));', '                    far = far.max(need(*old, *went, [old[along], old[along + 2]], *other, reaches[index]));', 'a_short_last_line_keeps_the_break_above_an_indented_paragraph'),
    Mutation("keep breaks: the room check reads one line's reach", 'src/textedit/layout.rs', '            .any(|(other, moves)| !*moves && lands((*old, new, *reach), *other, rule))', '            .any(|(other, moves)| !*moves && lands((*old, new, [old[0], old[2]]), *other, rule))', 'a_short_last_line_keeps_the_break_above_an_indented_paragraph'),
    Mutation('spread: a carried block is written only part of the way', 'src/textedit/layout.rs', 'along(0., -drop * part)', 'along(0., -drop + part * 0.)', 'the_added_lines_are_spread_over_the_breaks_below'),
    Mutation('wrap annotations: a popup is a mark', 'src/textedit.rs', '            .is_ok_and(|kind| kind == b"Popup")', '            .is_ok_and(|kind| kind == b"NoSuchSubtype")', 'an_annotation_over_the_moved_lines_refuses'),
    Mutation('wrap annotations: an unreadable list stops every edit', 'src/textedit.rs', '        annotations: annotation_rects(doc, id).ok(),', '        annotations: Some(annotation_rects(doc, id)?),', 'an_unreadable_annotation_list_refuses_a_wrap_and_nothing_else'),
    Mutation('wrap write: over an earlier push', 'src/textedit.rs', '                if shifts.contains_key(&(change.page, *show)) {\n                    return Err(layout::WRAP_CONFLICT.into());', '                if false {\n                    return Err(layout::WRAP_CONFLICT.into());', 'a_show_riding_a_pushed_line_is_not_also_moved_down_by_a_wrap'),
    Mutation('wrap write: under a later push', 'src/textedit.rs', '        if page.lowered.contains_key(&(*operator as usize)) {\n            return Err(layout::WRAP_CONFLICT.into());', '        if false {\n            return Err(layout::WRAP_CONFLICT.into());', 'a_line_pushed_by_one_edit_and_moved_down_by_a_wrap_is_refused'),
    Mutation('wrap write: never move the lines', 'src/textedit.rs', '                    .chain(lowered)\n', '', 'an_edit_past_the_page_edge_wraps_and_moves_the_rest_of_its_paragraph_down'),
    Mutation('wrap outlines: report no moved line', 'src/textedit.rs', '            placed.extend(value.placed.iter().copied());', '', 'placements_outline_the_wrapped_box_and_the_line_it_moved'),
    Mutation('geometric blocks: nothing is joined', 'src/textedit/blocks.rs', '            && joined(before, &segments[after], !taken.contains(&index), &drawn)', '            && false && joined(before, &segments[after], !taken.contains(&index), &drawn)', 'lines_at_one_pitch_one_font_and_one_left_edge_are_one_block'),
    Mutation('geometric blocks: any pitch is a line pitch', 'src/textedit/blocks.rs', 'const PITCH_MAX_EM: f64 = 3.0;', 'const PITCH_MAX_EM: f64 = 1e9;', 'a_line_in_another_font_or_size_or_too_far_below_starts_a_block'),
    Mutation('geometric blocks: fonts are not compared', 'src/textedit/blocks.rs', '    if before.face.is_none() || before.face != after.face {', '    if false {', 'a_line_in_another_font_or_size_or_too_far_below_starts_a_block'),
    Mutation('geometric blocks: a tie is a face', 'src/textedit/blocks.rs', '        (Some((face, _)), None) => Some(*face),', '        (Some((face, _)), _) => Some(*face),', 'a_line_in_another_font_or_size_or_too_far_below_starts_a_block'),
    Mutation('geometric blocks: a drawing between lines is nothing', 'src/textedit/blocks.rs', '    if drawn.iter().any(|g| {', '    if drawn.iter().any(|g| false && {', 'something_drawn_between_two_lines_ends_the_block_and_a_background_does_not'),
    Mutation('geometric blocks: a background ends the block', 'src/textedit/blocks.rs', '            && !(holds(*g, before.rect) && holds(*g, after.rect))\n', '\n', 'something_drawn_between_two_lines_ends_the_block_and_a_background_does_not'),
    Mutation('geometric blocks: an underline is between the lines', 'src/textedit/blocks.rs', 'const DESCENT_EM: f64 = 0.25;', 'const DESCENT_EM: f64 = 0.;', 'something_drawn_between_two_lines_ends_the_block_and_a_background_does_not'),
    Mutation("geometric blocks: between reaches past the lower line's capitals", 'src/textedit/blocks.rs', 'const CAP_EM: f64 = 0.75;', 'const CAP_EM: f64 = 1.5;', 'something_drawn_between_two_lines_ends_the_block_and_a_background_does_not'),
    Mutation('geometric blocks: left edges need not agree', 'src/textedit/blocks.rs', 'const LEFT_TOL: f64 = 1.0;', 'const LEFT_TOL: f64 = 100.;', 'left_edges_agree_or_the_first_line_is_indented_by_at_most_four_ems'),
    Mutation('geometric blocks: any indent is a first line', 'src/textedit/blocks.rs', 'const INDENT_MAX_EM: f64 = 4.0;', 'const INDENT_MAX_EM: f64 = 100.;', 'left_edges_agree_or_the_first_line_is_indented_by_at_most_four_ems'),
    Mutation('geometric blocks: any line may be a first line', 'src/textedit/blocks.rs', '            && joined(before, &segments[after], !taken.contains(&index), &drawn)', '            && joined(before, &segments[after], true, &drawn)', 'left_edges_agree_or_the_first_line_is_indented_by_at_most_four_ems'),
    Mutation('geometric blocks: any first line may hang', 'src/textedit/blocks.rs', '&& (delta > 0.) == before.label)', '&& (delta > 0. || !before.label))', 'left_edges_agree_or_the_first_line_is_indented_by_at_most_four_ems'),
    Mutation('geometric blocks: a label continues the block above', 'src/textedit/blocks.rs', '    if after.label {\n        return false;\n    }\n', '', 'a_list_label_starts_an_item_whose_first_line_hangs'),
    Mutation('geometric blocks: a row set with spaces is prose', 'src/textedit/blocks.rs', '        .any(|&i| pieces[i].gapped || pieces[i].text.contains("   "));', '        .any(|&i| pieces[i].gapped);', 'a_line_aligned_by_runs_of_spaces_is_a_table_row_and_not_prose'),
    Mutation('geometric blocks: a displacement inside a show is a word space', 'src/textedit/blocks.rs', '.any(|value| value <= -1000.)', '.any(|value| value <= -1e9)', 'a_line_aligned_by_runs_of_spaces_is_a_table_row_and_not_prose'),
    Mutation('geometric blocks: where a show starts is a column gap', 'src/textedit/blocks.rs', '                .skip(1)\n', '                .skip(0)\n', 'a_line_aligned_by_runs_of_spaces_is_a_table_row_and_not_prose'),
    Mutation("geometric blocks: the page's next line is the next line", 'src/textedit/blocks.rs', '            .filter(|s| s.line > before.line && overlaps(s))', '            .filter(|s| s.line == before.line + 1 && overlaps(s))', 'two_columns_whose_baselines_are_staggered_are_two_blocks_of_their_own_lines'),
    Mutation('geometric blocks: any pitch across another line', 'src/textedit/blocks.rs', 'const SKIP_PITCH_EM: f64 = 2.0;', 'const SKIP_PITCH_EM: f64 = 3.0;', 'across_another_line_of_the_page_a_pitch_over_two_ems_is_a_paragraph_gap'),
    Mutation('geometric blocks: no line across another line', 'src/textedit/blocks.rs', 'const SKIP_PITCH_EM: f64 = 2.0;', 'const SKIP_PITCH_EM: f64 = 0.;', 'across_another_line_of_the_page_a_pitch_over_two_ems_is_a_paragraph_gap'),
    Mutation('geometric blocks: the skipped-line pitch holds on every line', 'src/textedit/blocks.rs', '        if next > before.line + 1\n', '        if next > before.line\n', 'across_another_line_of_the_page_a_pitch_over_two_ems_is_a_paragraph_gap'),
    Mutation('geometric blocks: no gutter splits a line', 'src/textedit/blocks.rs', 'const GUTTER_EM: f64 = 1.0;', 'const GUTTER_EM: f64 = 1e9;', 'a_gutter_along_a_line_separates_two_blocks_side_by_side'),
    Mutation('geometric blocks: a pitch may step', 'src/textedit/blocks.rs', '            if seen.contains(&next) || pitch.is_some_and(|p: f64| (step - p).abs() > LEAD_TOL) {', '            if seen.contains(&next) {', 'a_block_ends_where_its_pitch_steps'),
    Mutation('geometric blocks: baselines must agree exactly', 'src/textedit/blocks.rs', 'const BASELINE_TOL: f64 = 0.5;', 'const BASELINE_TOL: f64 = 0.;', 'a_run_a_fraction_of_a_point_off_the_baseline_is_on_its_line'),
    Mutation('geometric blocks: a shifted run is a line of its own', 'src/textedit/blocks.rs', '    let groups = shifted(&pieces, groups);', '    let groups: Vec<(f64, Vec<usize>)> = groups.into_iter().map(|g| (pieces[g[0]].baseline, g)).collect();', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: a shifted run in the wrap is a line of its own', 'src/textedit/blocks.rs', '    let groups = shifted(&pieces, groups);', '    let groups: Vec<(f64, Vec<usize>)> = groups.into_iter().map(|g| (pieces[g[0]].baseline, g)).collect();', 'a_lowered_run_on_an_untagged_line_flows_with_it'),
    Mutation('geometric blocks: any shift is on the line', 'src/textedit/blocks.rs', 'const SHIFT_EM: f64 = 0.5;', 'const SHIFT_EM: f64 = 1.0;', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: no shift is on the line', 'src/textedit/blocks.rs', 'const SHIFT_EM: f64 = 0.5;', 'const SHIFT_EM: f64 = 0.1;', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: a column beside is set off a line', 'src/textedit/blocks.rs', '                let gutter = GUTTER_EM * size;\n', '                let gutter = f64::INFINITY;\n', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: the shorter line keeps the longer', 'src/textedit/blocks.rs', 'counts[other] > counts[index])', 'counts[other] < counts[index])', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: a line takes the baseline of its first run', 'src/textedit/blocks.rs', '        baseline,\n        left:', '        baseline: pieces[members[0]].baseline,\n        left:', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: a shifted run is on the further line', 'src/textedit/blocks.rs', '                off(a).total_cmp(&off(b))', '                off(b).total_cmp(&off(a))', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('geometric blocks: a shifted run is set off only the next baselines', 'src/textedit/blocks.rs', '        (0..groups.len())\n            .filter(|&other| counts[other] > counts[index])', '        [index.checked_sub(1), Some(index + 1)]\n            .into_iter()\n            .flatten()\n            .filter(|&other| other < groups.len() && counts[other] > counts[index])', 'a_run_raised_or_lowered_within_half_an_em_is_on_the_line_it_sits_in'),
    Mutation('labels: any number', 'src/textedit/blocks.rs', '    let digits = !body.is_empty() && body.len() <= 3 && body.chars().all(|c| c.is_ascii_digit());', '    let digits = !body.is_empty() && body.chars().all(|c| c.is_ascii_digit());', 'the_label_rule_reads_numbers_letters_numerals_and_bullets'),
    Mutation('labels: no numerals', 'src/textedit/blocks.rs', '    digits || letter || roman\n', '    digits || letter\n', 'the_label_rule_reads_numbers_letters_numerals_and_bullets'),
    Mutation('labels: no letters', 'src/textedit/blocks.rs', '    digits || letter || roman\n', '    digits || roman\n', 'the_label_rule_reads_numbers_letters_numerals_and_bullets'),
    Mutation('labels: no bullets', 'src/textedit/blocks.rs', '    if matches!(token, "•" | "·" | "▪" | "◦" | "‣" | "-" | "–" | "*" | "o")\n        || token.chars().count() == 1', '    if false\n        || token.chars().count() == 1', 'the_label_rule_reads_numbers_letters_numerals_and_bullets'),
    Mutation('labels: runs are joined with nothing', 'src/textedit/blocks.rs', '    let text = texts.collect::<Vec<_>>().join(" ");', '    let text = texts.collect::<Vec<_>>().concat();', 'the_label_rule_reads_numbers_letters_numerals_and_bullets'),
    Mutation('geometric blocks: tags are not consulted', 'src/textedit.rs', '    if inspection.blocks.is_empty() || blocks::FORCE.load(std::sync::atomic::Ordering::Relaxed) {', '    if true || blocks::FORCE.load(std::sync::atomic::Ordering::Relaxed) {', 'geometry_answers_only_where_the_tags_do_not'),
    Mutation('geometric blocks: never read off the geometry', 'src/textedit.rs', '    if inspection.blocks.is_empty() || blocks::FORCE.load(std::sync::atomic::Ordering::Relaxed) {', '    if false && blocks::FORCE.load(std::sync::atomic::Ordering::Relaxed) {', 'an_untagged_paragraph_wraps_by_the_blocks_its_lines_give'),
    Mutation('geometric blocks: every page is read off the geometry', 'src/textedit/blocks.rs', '        .is_some_and(|block| block.0 == 0)', '        .is_some_and(|block| block.0 == block.0)', 'text_beside_a_line_that_moves_refuses_the_wrap_on_an_untagged_page'),
    Mutation('geometric blocks: one line wraps', 'src/textedit/layout/wrap.rs', '    if !several && blocks::geometric_page(page) {', '    if false && blocks::geometric_page(page) {', 'a_grown_box_stops_at_the_edge_of_the_page'),
    Mutation('wrap room: text beside may come apart', 'src/textedit/layout.rs', '        if geometric\n            && (hits', '        if false\n            && (hits', 'text_beside_a_line_that_moves_refuses_the_wrap_on_an_untagged_page'),
    Mutation('wrap room: a drawing beside may come apart', 'src/textedit/layout.rs', '            }) || page.graphics.iter().any(|drawing| {', '            }) || page.graphics.iter().any(|drawing| false && {', 'text_beside_a_line_that_moves_refuses_the_wrap_on_an_untagged_page'),
    Mutation('wrap room: nothing is on one line', 'src/textedit/layout.rs', '    height > 0.1 && shared > height / 2.', '    height > 0.1 && shared > height * 2.', 'text_beside_a_line_that_moves_refuses_the_wrap_on_an_untagged_page'),
    Mutation('splice: a patch opening with a number runs into the operator before', 'src/textedit/streams.rs', "                output.push(b'\\n');", '', 'textedit_stream_patch_opening_with_a_number_is_kept_apart_from_the_operator_before'),
    ...
    Mutation('unembedded: forget the font box', 'src/textedit/fonts.rs', '        vertical_bounds: Some([bottom.min(0.), top.max(0.)]),\n        widths: result,', '        vertical_bounds: None,\n        widths: result,', 'textedit_unembedded_font_box_bounds_read_only_text'),
    ...
    Mutation('truetype: refuse a program without OS/2', 'src/textedit/fonts.rs', '[rights[0], rights[1]])));\n    }\n    Ok((face, restricted))', '[rights[0], rights[1]])));\n    } else {\n        return Err(invalid());\n    }\n    Ok((face, restricted))', 'textedit_macroman_requires_matching_encoding_and_honours_present_rights'),
    Mutation('dispatch: an unnamed CFF glyph still refuses the page', 'src/textedit/fonts.rs', 'Err(error) if error == cff::UNNAMED =>', 'Err(error) if false && error == cff::UNNAMED =>', 'textedit_cff_letter_outside_latin_keeps_its_text_read_only_and_the_page_editable'),
    Mutation('dispatch: every CFF refusal takes the glyph-name path', 'src/textedit/fonts.rs', 'Err(error) if error == cff::UNNAMED =>', 'Err(error) if !error.is_empty() =>', 'textedit_cff_letter_outside_latin_keeps_its_text_read_only_and_the_page_editable'),
    Mutation('restricted fonts: encode in them anyway', 'src/textedit/fonts.rs', 'if self.restricted && !text.is_empty() {', 'if false {', 'textedit_type1_rights_restrict_editing'),
    Mutation('restricted fonts: a deletion counts as a use', 'src/textedit/fonts.rs', 'if self.restricted && !text.is_empty() {', 'if self.restricted {', 'textedit_type1_rights_restrict_editing'),
    Mutation('restricted fonts: preview-and-print rights permit editing', 'src/textedit/fonts.rs', '    rights & !0x108 != 0\n', '    rights & !0x10c != 0\n', 'textedit_type1_rights_restrict_editing'),
    Mutation('restricted fonts: a restricted font writes no space', 'src/textedit/fonts.rs', '        self.codes_for(" ").is_ok()', '        self.encode(" ").is_ok()', 'textedit_cff_refuses_unvalidated_program_semantics_and_permissions'),
    Mutation('restricted fonts: TrueType rights ignored', 'src/textedit/fonts.rs', '        codes: codes.map(Codes::Single),\n    }\n    .restricted(restricted))', '        codes: codes.map(Codes::Single),\n    }\n    .restricted(false))', 'textedit_macroman_requires_matching_encoding_and_honours_present_rights'),
    Mutation('restricted fonts: CFF rights ignored', 'src/textedit/fonts/cff.rs', 'let restricted = profile::validate(&bytes)?.is_some_and(super::restricts);', 'let restricted = profile::validate(&bytes)?.is_some_and(|_| false);', 'textedit_cff_refuses_unvalidated_program_semantics_and_permissions'),
    Mutation('restricted fonts: CID-keyed CFF rights ignored', 'src/textedit/fonts/cff/cid.rs', '.is_ok_and(|rights| !super::super::restricts(rights))', '.is_ok()', 'textedit_cid_cff_refuses_restricted_rights'),
    Mutation('restricted fonts: composite rights ignored', 'src/textedit/fonts/composite.rs', '        codes: Some(Codes::Double(codes)),\n    }\n    .restricted(restricted))', '        codes: Some(Codes::Double(codes)),\n    }\n    .restricted(false))', 'textedit_composite_checks_embedding_rights_and_program_format'),
    Mutation('restricted fonts: the original font is written anyway', 'src/textedit/layout.rs', 'EditFont::Original if restricted => return Err(RESTRICTED_ORIGINAL.into()),', 'EditFont::Original if false => return Err(RESTRICTED_ORIGINAL.into()),', 'textedit_type1_rights_restrict_editing'),
    Mutation('restricted fonts: the preview does not say why', 'src/textedit/layout.rs', '            if restricted {\n                format!(', '            if false {\n                format!(', 'textedit_type1_rights_restrict_editing'),
    ...
    Mutation('image: forget marked image content', 'src/textedit.rs', 'tags.paint();\n            }\n            ("w", [value])', '// paint omitted\n            }\n            ("w", [value])', 'textedit_tagged_painted_content_preserves_structure_and_refuses_items_that_paint_nothing'),
...
MUTATIONS += [
    ...
    Mutation("boxed edit: allow collisions with other lines", "src/textedit/layout.rs",
        ...
        "                    if false && strikes(shown, *other, free.own) {",
