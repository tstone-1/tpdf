...
MUTATIONS = [
    ...
    Mutation('push: the preview crop stops at the box', 'src/textedit/layout.rs', '    for (_, other) in &placed {\n        extent = [', '    for (_, other) in &placed[..0] {\n        extent = [', 'the_preview_extent_covers_the_run_the_draft_pushes'),
    ...
    Mutation('wrap: text after the run flows anyway', 'src/textedit/layout/wrap.rs', '            if x > 0. && !other.text.trim().is_empty() {', '            if false {', 'text_after_the_run_on_its_line_keeps_the_page_edge_refusal'),
    ...
    Mutation('wrap: move another pending edit', 'src/textedit/layout.rs', '                    .any(|show| below.contains(&show))\n                {\n                    return Err(wrap::CONFLICT.into());', '                    .any(|show| below.contains(&show) && false)\n                {\n                    return Err(wrap::CONFLICT.into());', 'a_wrap_that_moves_another_pending_edit_is_refused_in_either_order'),
    ...
    Mutation('wrap: land on the moved lines', 'src/textedit/layout.rs', '                            shows_of(page, id).iter().any(|show| below.contains(show)),', '                            false,', 'an_edit_past_the_page_edge_wraps_and_moves_the_rest_of_its_paragraph_down'),
    Mutation('wrap: move the lines by a pitch too few', 'src/textedit/layout.rs', '                let drop = lines.saturating_sub(1) as f64 * plan.pitch;', '                let drop = lines.saturating_sub(2) as f64 * plan.pitch;', 'an_edit_past_the_page_edge_wraps_and_moves_the_rest_of_its_paragraph_down'),
