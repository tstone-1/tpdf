...
MUTATIONS = [
    ...
    Mutation('stroked lines: skip point coordinate bounds', 'src/textedit/clipping.rs', 'if point\n            .iter()\n            .any(|value| !value.is_finite() || value.abs() > 1_000_000.)', 'if false', 'textedit_stroked_lines_refuse_invalid_and_incomplete_paths_atomically'),
    Mutation('stroked lines: accept extra point operands', 'src/textedit/clipping.rs', 'if op.operands.len() != 2 {', 'if op.operands.len() < 2 {', 'textedit_stroked_lines_refuse_invalid_and_incomplete_paths_atomically'),
    Mutation('stroked lines: accept stroke operands', 'src/textedit/clipping.rs', '|| !end.operands.is_empty() {', '{', 'textedit_stroked_lines_refuse_invalid_and_incomplete_paths_atomically'),
    ...
    Mutation('stroked lines: consume following operator', 'src/textedit/clipping.rs', 'Ok(count + 1)', 'Ok(count + 2)', 'textedit_stroked_lines_preserve_graphics_and_following_text'),
    ...
    Mutation('stroked lines: accept empty subpath', 'src/textedit/clipping.rs', 'if count < 2 {', 'if count < 1 {', 'textedit_stroked_lines_refuse_invalid_and_incomplete_paths_atomically'),
    ...
    Mutation('painted rectangles: discard an established clip', 'src/textedit.rs', 'path_until = index + 2;', 'path_until = index + 2; clip = None;', 'textedit_painted_rectangles_do_not_replace_or_discard_clipping'),
    Mutation('painted rectangles: consume a following operator', 'src/textedit.rs', 'path_until = index + 2;', 'path_until = index + 3;', 'textedit_painted_rectangles_preserve_paint_state_and_surrounding_operators'),
    Mutation('painted rectangles: count discarded paths as tagged content', 'src/textedit.rs', 'if content.operations[index + 1].operator != "n" {', 'if true {', 'textedit_tagged_painted_content_preserves_structure_and_refuses_empty_items'),
    Mutation('painted rectangles: omit tagged paint content', 'src/textedit.rs', 'if content.operations[index + 1].operator != "n" {\n                        tags.paint();\n                    }', '// rectangle paint omitted', 'textedit_tagged_painted_content_preserves_structure_and_refuses_empty_items'),
    ...
    Mutation('single ranges: omit reversed-endpoint guard', 'src/textedit/fonts/mapping.rs', 'if last < first || *target < 32 || end_target > 126 {', 'if *target < 32 || end_target > 126 {', 'textedit_single_ranges_reject_bad_endpoints_counts_and_overlap'),
    Mutation('single ranges: allow control targets', 'src/textedit/fonts/mapping.rs', '*target < 32', 'false', 'textedit_single_ranges_reject_bad_endpoints_counts_and_overlap'),
    Mutation('single ranges: allow expansion past ASCII', 'src/textedit/fonts/mapping.rs', 'end_target > 126', 'end_target > 127', 'textedit_single_ranges_reject_bad_endpoints_counts_and_overlap'),
    Mutation('single ranges: drop range endpoint', 'src/textedit/fonts/mapping.rs', 'for code in first..=last {\n                let ch = *target', 'for code in first..last {\n                let ch = *target', 'textedit_single_ranges_expand_exactly_and_mix_with_bfchar'),
    ...
    Mutation("textedit: forget saved fill colour space", "src/textedit.rs", "states.push((\n                    selected_font,\n                    leading,\n                    page_transform,\n                    fill_components,\n                    clip,\n                    spacing,\n                ));", "states.push((selected_font, leading, page_transform, 1, clip, spacing));", "textedit_colours_restore_state_and_preserve_operators"),
    ...
    Mutation("textedit: forget saved page translation", "src/textedit.rs", 'states.push((\n                    selected_font,\n                    leading,\n                    page_transform,\n                    fill_components,\n                    clip,\n                    spacing,\n                ));', 'states.push((selected_font, leading, [1., 0., 0., 1., 0., 0.], fill_components, clip, spacing));', "textedit_page_translations_compose_restore_and_preserve_following_runs"),
    ...
    Mutation("textedit: accept custom word spacing or rise", "src/textedit.rs", '("Tw" | "Ts", [value]) if number(value)? == 0.0', '("Tw" | "Ts", [_])', "textedit_default_setters_refuse_nondefault_and_malformed_operands"),
    ...
    Mutation('spacing: ignore spacing in kerning arrays', 'src/textedit.rs', 'metrics.spaced_layout(&fragment, size, spacing)?', 'metrics.spaced_layout(&fragment, size, 0.)?', 'textedit_spacing_measures_character_steps_and_tj_fragments'),
    Mutation('spacing: ignore spacing when writing', 'src/textedit.rs', 'metrics.spaced_layout(&change.replacement, run.size, spacing)?', 'metrics.spaced_layout(&change.replacement, run.size, 0.)?', 'textedit_spacing_replacement_advance_and_ink_are_both_checked_atomically'),
