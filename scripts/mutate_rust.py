...
MUTATIONS = [
    ...
    Mutation('annotations: offer link text for editing', 'src/textedit/tagging.rs', 'Some(Some(mcid)) => annotation_owner(&self.names[mcid]) || self.names[mcid] == ORPHAN,', 'Some(Some(mcid)) => self.names[mcid] == ORPHAN,', 'textedit_link_text_stays_read_only_while_neighbouring_text_is_edited'),
    Mutation('orphans: offer orphaned text for editing', 'src/textedit/tagging.rs', 'Some(Some(mcid)) => annotation_owner(&self.names[mcid]) || self.names[mcid] == ORPHAN,', 'Some(Some(mcid)) => annotation_owner(&self.names[mcid]),', 'textedit_orphaned_parent_tree_slots_stay_read_only_and_cannot_hide_reachable_owners'),
