...
MUTATIONS = [
    ...
    Mutation('preserved forms: lose operation bound', 'src/textedit/forms.rs', 'if budget.operations > MAX_OPERATIONS {', 'if false {', 'textedit_preserved_forms_refuse_cycles_external_carriers_and_unbalanced_state'),
    ...
    Mutation('tag containers: discard immediate parent', 'src/textedit/tagging.rs', 'element(doc, child, parent_id, &scope, owns_text)?', 'element(doc, child, root_id, &scope, owns_text)?', 'textedit_containers_keep_interleaved_page_owners_and_nested_leaves'),
    ...
    Mutation('nested tags: inherit an ancestor page', 'src/textedit/tagging.rs', '                !annotation_owner(name(get(child, b"S")?)?),\n            )?;', '                !annotation_owner(name(get(child, b"S")?)?),\n            )?\n            .or(plain.page);', 'textedit_nested_optional_container_pages_and_scalar_children_are_explicit'),
    ...
    Mutation('alt: admit alternate text on paragraphs', 'src/textedit/tagging.rs', 'if matches!(tag, b"Figure" | b"Link" | b"Form") {\n        allowed.push(b"Alt");', 'if true {\n        allowed.push(b"Alt");', 'textedit_link_titles_and_alternate_text_are_preserved_on_read_only_owners'),
    Mutation('titles: admit titles on editable text', 'src/textedit/tagging.rs', 'let title_limit = if owns_text { 0 } else { 4096 };', 'let title_limit = 4096;', 'textedit_link_titles_and_alternate_text_are_preserved_on_read_only_owners'),
    ...
    Mutation('tagged: offer artifact text for editing', 'src/textedit/tagging.rs', '            _ => !self.names.is_empty(),', '            _ => false,', 'textedit_tagged_artifact_and_unmarked_text_remain_read_only'),
    ...
    Mutation('textedit: forget saved fill colour space', 'src/textedit.rs', "states.push((\n                    selected_font,\n                    leading,\n                    page_transform,\n                    fill_components,\n                    clip,\n                    spacing,\n                    word_spacing,\n                    stroke_components,\n                    compound_clips.clone(),\n                ));", "states.push((selected_font, leading, page_transform, patterns::Colour::Solid(1), clip, spacing, word_spacing, stroke_components, compound_clips.clone()));", 'textedit_colours_restore_state_and_preserve_operators'),
    ...
    Mutation('bounded table: forget the ink bounds it declared', 'src/textedit/tagging.rs', 'let bounded = bounded || (role == b"Table" && bounds(doc, child));', 'let bounded = bounded;', 'textedit_bounded_tables_keep_their_ink_bounds_and_stop_offering_cell_text'),
    Mutation('bounded table: look for the bounds on the wrong element', 'src/textedit/tagging.rs', 'let bounded = bounded || (role == b"Table" && bounds(doc, child));', 'let bounded = bounded || (role == b"TR" && bounds(doc, child));', 'textedit_bounded_tables_keep_their_ink_bounds_and_stop_offering_cell_text'),
    Mutation('bounded table: leave its content editable', 'src/textedit/tagging.rs', '                    if bounded {\n                        bounded_slots.entry(owner).or_default().insert(mcid);\n                    }', '', 'textedit_bounded_tables_keep_their_ink_bounds_and_stop_offering_cell_text'),
    ...
    Mutation('bounded table: read the bounds from anywhere', 'src/textedit/tagging.rs', '.is_ok_and(|value| value.as_dict().is_ok_and(|entries| entries.has(b"BBox")))', '.is_ok()', 'textedit_bounded_tables_keep_their_ink_bounds_and_stop_offering_cell_text'),
    Mutation('nested list: keep a sublist out of the list body', 'src/textedit/tagging.rs', '            if plain.tag == b"LBody"\n                && node(doc, *id)\n                    .is_ok_and(|child| get(child, b"S").and_then(name).is_ok_and(|tag| tag == b"L"))\n            {\n                nested.push((item, plain.id));\n                continue;\n            }\n', '', 'textedit_lists_nested_in_a_list_body_keep_every_level_editable'),
    Mutation('nested list: defer any child of a list body', 'src/textedit/tagging.rs', 'get(child, b"S").and_then(name).is_ok_and(|tag| tag == b"L")', 'true', 'textedit_list_bodies_still_validate_their_own_inline_leaves'),
    Mutation('nested list: defer a child of any element', 'src/textedit/tagging.rs', 'if plain.tag == b"LBody"\n                && node(doc, *id)', 'if !plain.tag.is_empty()\n                && node(doc, *id)', 'textedit_paragraphs_refuse_a_list_among_their_content'),
    ...
    Mutation('soft mask: skip alpha validation', 'src/textedit/images.rs', 'Ok(bytes + samples(doc, None, mask, remaining - bytes)?)', 'Ok(bytes)', 'textedit_soft_masks_refuse_colour_palettes_and_masks_of_their_own'),
