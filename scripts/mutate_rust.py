...
MUTATIONS = [
    ...
    Mutation('keep breaks: a blank line is a whole pitch', 'src/textedit/layout.rs', '    let blank = pitch - (slack[cross] - 0.1) - give * pitch;', '    let blank = pitch - give * pitch;', 'a_paragraph_with_no_room_below_is_refused_with_the_reason'),
    ...
    Mutation('wrap room: a drawing beside may come apart', 'src/textedit/layout.rs', '            }) || page.graphics.iter().enumerate().any(|(index, drawing)| {', '            }) || page.graphics.iter().enumerate().any(|(index, drawing)| false && {', 'text_beside_a_line_that_moves_refuses_the_wrap_on_an_untagged_page'),
