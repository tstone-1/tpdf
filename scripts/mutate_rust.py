...
MUTATIONS = [
    ...
    Mutation('push: measure the move from the room instead', 'src/textedit/layout.rs', '    let from = pushed\n        .iter()\n        .map(|(rect, _)| axis.at(axis.edge(*rect, false)))\n        .fold(f64::INFINITY, f64::min)\n        .max(width);', '    let from = free;', 'the_push_is_measured_from_the_run_it_moves_not_from_the_room'),
    ...
    Mutation('columns: a column is pushed along', 'src/textedit/layout.rs', 'if grow && pushable(page, run, id) && !columns.contains(&id) =>', 'if grow && pushable(page, run, id) =>', 'a_line_that_reaches_the_next_column_wraps_on_an_untagged_page'),
