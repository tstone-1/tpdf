...
def generated() -> set[str]:
    ...
    Read from the module's own tables, not from its text. A regex over the whole
    file counts a path written in a *comment* --- and `ci_fixtures.py` has a
    docstring and a dozen comments naming fixtures, so a path mentioned in prose
    and generated by nothing would have made a bogus workflow step look
    generated, which is the one direction this gate exists to refuse. It is the
    trap about a scanner reading its own prose, arriving on the other side of
    the same check: `wanted()` below already scans what executes rather than
    what is written, and this half did not.

    It was wrong the other way round too, and that half was measured rather than
    reasoned about: the text scan saw 11 stems where the tables hold 37, because
    `SIGNED` and `HOSTILE` build their paths as `f"testdata/{name}.pdf"` and a
    regex cannot read an f-string. A workflow step naming any signed or hostile
    fixture was therefore reported as ungeneratable, which is a false alarm on a
    step that runs perfectly well.
    """
    spec = importlib.util.spec_from_file_location("ci_fixtures", FIXTURES)
    if spec is None or spec.loader is None:
        raise SystemExit(f"[FAIL] {FIXTURES} could not be loaded as a module")
    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
    except Exception as why:  # noqa: BLE001 -- reported, never a traceback
        # A traceback is the one verdict shape that names no check, so the run
        # that found the defect would report a stack and the reader learn
        # nothing.
        raise SystemExit(f"[FAIL] could not read {FIXTURES.name}: {why!r}")
    stems: set[str] = set()
    for table in ("FIXTURES", "SIGNED", "HOSTILE"):
        for artifact, _command in getattr(module, table, []):
            # The same pattern the workflow side is read with, so the two halves
            # cannot disagree about what a path looks like: `testdata/abort/
            # xref-bomb.pdf` matches neither, and is asked for by neither.
            stems |= set(PATH.findall(artifact))
    return stems
