...
vi.mock("@tauri-apps/api/core", () => ({
  convertFileSrc: (path: string, protocol: string) => convertFileSrc(path, protocol),
}));

const { resetTileOrigin, tileOrigin, tileUrl } = await import("./tiles");
...
describe("a tile url", () => {
  ...
  it("is built on whatever origin this platform serves", () => {
    shape = "scheme";
    expect(tileUrl(request)).toBe("tile://localhost/3/7/1500/10/20/512/512?fmt=raw");
    resetTileOrigin();
    shape = "http";
    expect(tileUrl(request)).toBe("http://tile.localhost/3/7/1500/10/20/512/512?fmt=raw");
  });
