...
import { SIGNATURE_KEY } from "./signature";
...
export async function signatureCheck(host: OpenCheckHost, expected: string, report: Report): Promise<void> {
  ...
  const savedBefore = localStorage.getItem(SIGNATURE_KEY);
  ...
  const place = async () => {
    button("Place signature").click(); await host.idle();
  ...
  check("an empty signature cannot be placed", button("Place signature").disabled);
  ...
  check("drawing enables signature placement", !button("Place signature").disabled);
  await place();
  ...
  if (!await settle(() => !button("Place signature").disabled, 3000)) throw new Error("import did not finish");
  await place();
  check("imported pixels create a second signature", host.edits()!.state.marks.length === 2 && host.edits()!.state.marks.every((m) => !!m.image));
  check("local reuse is opt-in", localStorage.getItem(SIGNATURE_KEY) === savedBefore);
  ...
  try {
    ...
    localStorage.setItem(SIGNATURE_KEY, JSON.stringify(sample));
    button("Use saved signature").click();
    check("a saved signature is ready to place", !button("Place signature").disabled);
    ...
    await place();
    check("remembering stores the accepted pixels", JSON.parse(localStorage.getItem(SIGNATURE_KEY)!).rgba.length > 0);
    host.run("edit.undo"); await host.idle();
    host.run("edit.addSignature");
    if (!await settle(() => !!dialog(), 3000)) throw new Error("forget dialog did not open");
    button("Forget saved signature").click();
    check("the saved signature can be forgotten", localStorage.getItem(SIGNATURE_KEY) === null);
    button("Cancel").click();
  } finally {
    if (savedBefore === null) localStorage.removeItem(SIGNATURE_KEY);
    else localStorage.setItem(SIGNATURE_KEY, savedBefore);
  }
